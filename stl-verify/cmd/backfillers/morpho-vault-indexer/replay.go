...
func replayPartition(
	...
	for _, e := range entries {
		blockTimestamp, err := tsCache.timestampAt(ctx, e.blockNumber)
		if err != nil {
			return err
		}
...
func collectPartitionV2Logs(
	...
	for _, key := range receiptKeys {
		...
		blockEntries, err := filterV2Logs(receipts, parsed.BlockNumber, v2Vaults, topics)
		if err != nil {
			return nil, fmt.Errorf("filtering %s: %w", key, err)
		}
		for i := range blockEntries {
			blockEntries[i].blockVersion = parsed.Version
		}
...
func newBlockTimestampCache(fetcher headerTimeFetcher) *blockTimestampCache {
	return &blockTimestampCache{fetcher: fetcher, cache: make(map[int64]time.Time)}
}

func (c *blockTimestampCache) timestampAt(ctx context.Context, blockNumber int64) (time.Time, error) {
	if ts, ok := c.cache[blockNumber]; ok {
		return ts, nil
	}
	header, err := c.fetcher.HeaderByNumber(ctx, big.NewInt(blockNumber))
	if err != nil {
