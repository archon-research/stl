...
func TestParseConfig(t *testing.T) {
	tests := []struct {
		...
		{
			name: "MULTICALL_CHUNK_SIZE env override",
			args: []string{"-queue", "https://sqs.us-east-1.amazonaws.com/123/q", "-db", "postgres://localhost/db", "-redis", "localhost:6379"},
			envVars: map[string]string{
				"ALCHEMY_API_KEY":      "test-key",
			...
			wantCfg: cliConfig{
				...
				visibilityTimeout:  300,
				chainID:            1,
				multicallChunkSize: 250,
			},
		},
		{
			name: "invalid MULTICALL_CHUNK_SIZE",
			args: []string{"-queue", "https://sqs.us-east-1.amazonaws.com/123/q", "-db", "postgres://localhost/db", "-redis", "localhost:6379"},
			envVars: map[string]string{
				"ALCHEMY_API_KEY":      "test-key",
				"MULTICALL_CHUNK_SIZE": "abc",
			},
			wantError: "parsing MULTICALL_CHUNK_SIZE",
		},
	}
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			for _, key := range []string{
				...
				"S3_BUCKET", "DEPLOY_ENV", "MULTICALL_CHUNK_SIZE",
			} {
			...
			if cfg.multicallChunkSize != tt.wantCfg.multicallChunkSize {
				t.Errorf("multicallChunkSize: expected %d, got %d", tt.wantCfg.multicallChunkSize, cfg.multicallChunkSize)
			}
		})
	}
}
