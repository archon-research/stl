...
func run(ctx context.Context, args []string) error {
	...
	if err != nil {
		return fmt.Errorf("creating maple telemetry: %w", err)
	}
	...
	mapleRepo, err := postgres.NewMapleRepository(pool, logger, buildReg.BuildID())
	if err != nil {
		return fmt.Errorf("creating maple repository: %w", err)
	}

	// Service
	svcConfig := maple_indexer.Config{
