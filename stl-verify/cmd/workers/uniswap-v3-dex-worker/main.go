...
func parseConfig(args []string) (cliConfig, error) {
	fs := flag.NewFlagSet("uniswap-v3-dex-worker", flag.ContinueOnError)
	...
	if chainIDStr == "" {
		return cliConfig{}, fmt.Errorf("CHAIN_ID environment variable is required (no silent default to mainnet)")
	}
	...
	// NFPM address: default to mainnet NFPM if unset.
	nfpmStr := env.Get("NFPM_ADDRESS", "")
	if nfpmStr == "" {
		cfg.nfpmAddress = uniswap_v3_dex.DefaultNFPMAddress
	} else {
		if !common.IsHexAddress(nfpmStr) {
			return cliConfig{}, fmt.Errorf("NFPM_ADDRESS %q is not a valid hex address", nfpmStr)
		}
		cfg.nfpmAddress = common.HexToAddress(nfpmStr)
	}
...
func run(ctx context.Context, args []string) error {
	...
	logger.Info("starting uniswap v3 dex worker",
		"queue", cfg.queueURL,
		"redis", cfg.redisAddr,
		"chainID", cfg.chainID,
		"nfpm", cfg.nfpmAddress.Hex(),
	...
	uniswapRepo, err := postgres.NewUniswapV3PoolRepository(pool, logger, buildReg.BuildID())
	if err != nil {
		return fmt.Errorf("creating uniswap v3 pool repository: %w", err)
	}

	svcConfig := uniswap_v3_dex.Config{
		SQSConsumerConfig: shared.SQSConsumerConfig{
			MaxMessages: cfg.maxMessages,
			Logger:      logger,
			ChainID:     cfg.chainID,
		},
		NFPMAddress: cfg.nfpmAddress,
	}
	...
	logger.Info("uniswap v3 dex worker started, waiting for messages...")
