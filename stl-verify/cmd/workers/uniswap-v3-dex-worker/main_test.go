...
func TestParseConfig(t *testing.T) {
	customNFPM := "0xAaAAAaAaaAaAaaaAaAAAAAaaAAaAaAaAaAaAaaAa"
	tests := []struct {
		name      string
		args      []string
		...
		{
			name: "env vars only, default NFPM",
			args: []string{},
			envVars: map[string]string{
		...
		{
			name: "custom NFPM_ADDRESS override",
			args: []string{
			...
			wantCfg: cliConfig{
				...
				s3Bucket:          "my-bucket",
				deployEnv:         "test",
				nfpmAddress:       common.HexToAddress(customNFPM),
				maxMessages:       10,
				waitTime:          20,
				visibilityTimeout: 300,
				chainID:           1,
		...
		},
		{
			...
			envVars: map[string]string{
				...
				"S3_BUCKET":       "my-bucket",
				"DEPLOY_ENV":      "test",
				"NFPM_ADDRESS":    "notahex",
				"CHAIN_ID":        "1",
			},
			wantError: "NFPM_ADDRESS",
		},
		{
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			for _, key := range []string{
				...
				"S3_BUCKET", "DEPLOY_ENV", "NFPM_ADDRESS",
			} {
			...
			if cfg.nfpmAddress != tt.wantCfg.nfpmAddress {
				t.Errorf("nfpmAddress = %s, want %s", cfg.nfpmAddress.Hex(), tt.wantCfg.nfpmAddress.Hex())
			}
