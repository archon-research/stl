...
func TestParseConfig(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			// chainutil.RequireChainID). Subtests that don't exercise
			// chain-ID handling get a sensible default so the parse can
			// reach the field they DO care about. Subtests that
			// explicitly set CHAIN_ID (valid or invalid) keep that value.
