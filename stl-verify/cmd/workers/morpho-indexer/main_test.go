...
func TestParseConfig(t *testing.T) {
	tests := []struct {
		...
		{
			name: "MORPHO_SYMBOL_SWEEP_INTERVAL_BLOCKS env override",
			args: []string{"-queue", "https://sqs.us-east-1.amazonaws.com/123/q", "-db", "postgres://localhost/db", "-redis", "localhost:6379"},
			envVars: map[string]string{
				"ALCHEMY_API_KEY":                     "test-key",
				"S3_BUCKET":                           "my-bucket",
				"DEPLOY_ENV":                          "sentinelstaging",
				"MORPHO_SYMBOL_SWEEP_INTERVAL_BLOCKS": "50",
			},
			wantCfg: cliConfig{
				queueURL:                  "https://sqs.us-east-1.amazonaws.com/123/q",
				dbURL:                     "postgres://localhost/db",
				...
				visibilityTimeout:         300,
				chainID:                   1,
				symbolSweepIntervalBlocks: 50,
				symbolBackstopBlocks:      1000,
			},
		},
		{
			name: "MORPHO_SYMBOL_BACKSTOP_BLOCKS env override",
			args: []string{"-queue", "https://sqs.us-east-1.amazonaws.com/123/q", "-db", "postgres://localhost/db", "-redis", "localhost:6379"},
			envVars: map[string]string{
				"ALCHEMY_API_KEY":               "test-key",
				"S3_BUCKET":                     "my-bucket",
				"DEPLOY_ENV":                    "sentinelstaging",
				"MORPHO_SYMBOL_BACKSTOP_BLOCKS": "5000",
			},
			wantCfg: cliConfig{
				queueURL:                  "https://sqs.us-east-1.amazonaws.com/123/q",
				dbURL:                     "postgres://localhost/db",
				...
				chainID:                   1,
				symbolSweepIntervalBlocks: 10,
				symbolBackstopBlocks:      5000,
			},
		},
		{
			name: "invalid MORPHO_SYMBOL_SWEEP_INTERVAL_BLOCKS",
			args: []string{"-queue", "https://sqs.us-east-1.amazonaws.com/123/q", "-db", "postgres://localhost/db", "-redis", "localhost:6379"},
			envVars: map[string]string{
				"ALCHEMY_API_KEY":                     "test-key",
				"MORPHO_SYMBOL_SWEEP_INTERVAL_BLOCKS": "not-a-number",
			},
			wantError: "parsing MORPHO_SYMBOL_SWEEP_INTERVAL_BLOCKS",
		},
		{
			name: "invalid MORPHO_SYMBOL_BACKSTOP_BLOCKS",
			args: []string{"-queue", "https://sqs.us-east-1.amazonaws.com/123/q", "-db", "postgres://localhost/db", "-redis", "localhost:6379"},
			envVars: map[string]string{
				"ALCHEMY_API_KEY":               "test-key",
				"MORPHO_SYMBOL_BACKSTOP_BLOCKS": "bad",
			},
			wantError: "parsing MORPHO_SYMBOL_BACKSTOP_BLOCKS",
		},
		{
			name: "negative MORPHO_SYMBOL_SWEEP_INTERVAL_BLOCKS",
			args: []string{"-queue", "https://sqs.us-east-1.amazonaws.com/123/q", "-db", "postgres://localhost/db", "-redis", "localhost:6379"},
			envVars: map[string]string{
				"ALCHEMY_API_KEY":                     "test-key",
				"MORPHO_SYMBOL_SWEEP_INTERVAL_BLOCKS": "-5",
			},
			wantError: "MORPHO_SYMBOL_SWEEP_INTERVAL_BLOCKS must be >= 0",
		},
		{
			name: "negative MORPHO_SYMBOL_BACKSTOP_BLOCKS",
			args: []string{"-queue", "https://sqs.us-east-1.amazonaws.com/123/q", "-db", "postgres://localhost/db", "-redis", "localhost:6379"},
			envVars: map[string]string{
				"ALCHEMY_API_KEY":               "test-key",
				"MORPHO_SYMBOL_BACKSTOP_BLOCKS": "-5",
			},
			wantError: "MORPHO_SYMBOL_BACKSTOP_BLOCKS must be >= 0",
		},
	}
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			for _, key := range []string{
				...
				"MORPHO_SYMBOL_SWEEP_INTERVAL_BLOCKS", "MORPHO_SYMBOL_BACKSTOP_BLOCKS",
			} {
			...
			if cfg.symbolSweepIntervalBlocks != tt.wantCfg.symbolSweepIntervalBlocks {
				t.Errorf("symbolSweepIntervalBlocks: expected %d, got %d", tt.wantCfg.symbolSweepIntervalBlocks, cfg.symbolSweepIntervalBlocks)
			}
			if cfg.symbolBackstopBlocks != tt.wantCfg.symbolBackstopBlocks {
				t.Errorf("symbolBackstopBlocks: expected %d, got %d", tt.wantCfg.symbolBackstopBlocks, cfg.symbolBackstopBlocks)
			}
		})
	}
}
