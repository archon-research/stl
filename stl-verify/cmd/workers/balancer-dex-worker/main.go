...
func parseConfig(args []string) (cliConfig, error) {
	fs := flag.NewFlagSet("balancer-dex-worker", flag.ContinueOnError)
...
func run(ctx context.Context, args []string) error {
	...
	logger.Info("starting balancer dex worker",
		"queue", cfg.queueURL,
		"redis", cfg.redisAddr,
		"chainID", cfg.chainID,
	...
	balancerRepo, err := postgres.NewBalancerPoolRepository(pool, logger, buildReg.BuildID())
	if err != nil {
		return fmt.Errorf("creating balancer pool repository: %w", err)
	}

	svcConfig := balancer_dex.Config{
	...
	logger.Info("balancer dex worker started, waiting for messages...")
