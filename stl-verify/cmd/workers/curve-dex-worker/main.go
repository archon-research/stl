...
func parseConfig(args []string) (cliConfig, error) {
	fs := flag.NewFlagSet("curve-dex-worker", flag.ContinueOnError)
...
func run(ctx context.Context, args []string) error {
	...
	logger.Info("starting curve dex worker",
		"queue", cfg.queueURL,
		"redis", cfg.redisAddr,
		"chainID", cfg.chainID,
	...
	curveRepo, err := postgres.NewCurvePoolRepository(pool, logger, buildReg.BuildID())
	if err != nil {
		return fmt.Errorf("creating curve pool repository: %w", err)
	}

	svcConfig := curve_dex.Config{
	...
	logger.Info("curve dex worker started, waiting for messages...")
