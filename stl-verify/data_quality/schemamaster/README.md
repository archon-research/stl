...
`derived` marks a table whose rows are computed from another governed table rather than ingested: the `*_current` caches (VEC-577), which a trigger keeps at the newest history row per key. Their columns are copies of already-canonical history columns, so the per-column and nullability checks apply in full; they are not observations, so the required-key rules and transform coverage do not. A `derived` table declares canonical widths even where the history it copies kept a narrower pre-convention type (`token_price_current.oracle_id` is `int8` and `block_version` `int4`, against `int2` on `onchain_token_price`), so it needs no type exemption.
