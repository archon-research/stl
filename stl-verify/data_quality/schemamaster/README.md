...
- the enrichment master and resolver layer — curated append-only SCD2 masters and instrument/code resolvers, not part of the raw-to-canonical transform layer this register governs. Their columns (natural keys `processing_version`, `valid_from`, and the classification/attribute columns) are introduced by the enrichment layer, not the transform vocabulary, and are catalogued by each table's own migration `COMMENT`s. The masters carry no surrogate key: identity is the natural key `(id, processing_version)`, resolved via each `*_current` view. The registered entries are `security_master` and `security_instrument_bridge` (VEC-412), the entity side `entity_master` / `entity_ref_codes` (VEC-410 / VEC-414), and the position side `position_classification` / `position_entity_link` (VEC-401 / VEC-415) plus `position_state` (VEC-402, the shared spine the per-protocol position materializers write into). Each was added to `ignore_tables` by the PR that created it, for the same reason: it sits outside the transform vocabulary. A table the check has not yet been told to ignore is simply never seen, since it only scans live `BASE TABLE`s. The per-protocol projection *views* (e.g. `position_morpho_market`) are not base tables, so the check never sees them either.
