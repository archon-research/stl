...
export function matchesSearchQuery(searchString: string | null | undefined, query: string | null | undefined): boolean {
...
export function buildRowSearchString(fields: (string | number | null | undefined)[]): string {
  return normalizeSearchString(fields.filter((f) => f !== null && f !== undefined).map(String).join(' '));
}

export function validateSortingState(sorting: unknown): SortingState {
  if (Array.isArray(sorting) && sorting.every((item) => typeof item === 'object' && item !== null && 'id' in item && 'desc' in item && typeof (item as Record<string, unknown>).id === 'string' && typeof (item as Record<string, unknown>).desc === 'boolean')) {
