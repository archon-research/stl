...
async function main() {
  ...
  try {
    const { balancedColumns } = await vite.ssrLoadModule('/src/lib/dashboard.ts');
    ...
    assert.equal(balancedColumns(6, 2), 2, 'six cards at two columns is 2 + 2 + 2');
    ...
    console.log('metrics grid columns balance rows without adding one.');
  } finally {
    await vite.close();
  }
}
