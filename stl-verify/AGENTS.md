...
- **Testing**:
    ...
    - **One service set per CI run, never per test** — service startup and migrations, not the test, dominate integration-test CI time. Start each service in `TestMain` via `testutil.Start{TimescaleDB,Redis,LocalStack}ForMain` and keep it in a package var (`sharedDSN`, `sharedRedisAddr`, `sharedLocalStackCfg`). In CI those helpers return the shard's `services:` container (via `STL_TEST_POSTGRES_DSN`, `STL_TEST_REDIS_ADDR`, `STL_TEST_LOCALSTACK_ENDPOINT`); locally they fall back to a testcontainer, so nothing extra is needed to run tests by hand. The Postgres server those variables name must be disposable: the suite creates and drops databases on it freely, so never point them at a dev database you care about. Never start a container in a test: `make shared-container-check` enforces this in `ci-checks`.
    - **Isolate each test inside those services**: `testutil.SetupTestDB(t, sharedDSN)` for Postgres, a `testutil.SanitizeTestName(t.Name())` prefix for Redis keys and SQS/SNS names, `testutil.S3TestBucketName(t, prefix)` for buckets, `testutil.SQSTestFifoQueueName(t, prefix)` for FIFO queues. Anything a test counts (objects, messages, rows) needs its own database/bucket/queue, not a shared one. A test that drives a binary cannot namespace the names the binary builds for itself — its Redis cache key (chain/block/version) and its raw S3 bucket — so it takes its block number from `testutil.ReservedBlock(t, "<package>")` (the registry in `internal/testutil/reservedblocks.go`, one block per package), creates that bucket with `testutil.EnsureBucket` because a sibling package asks for the same name, and never counts objects in it. `make ci-service-check` holds the workflow's service images and LocalStack `SERVICES` to what the test helpers ask for.
    - **`SetupTestDB` clones a migrated template database**, so a new test costs a file copy rather than a full migration pass, and migration work stays flat as tests are added. Use `testutil.SetupDBForMain(baseDSN, name)` for a database shared by one test file from `TestMain`. Never migrate per test. The template's name carries a digest of the migration set, so switching branches on a long-lived `STL_TEST_POSTGRES_DSN` server leaves the previous one behind; `make test-templates-clean` removes them, by hand because dropping one from inside the suite would race a sibling process mid-clone. `db/migrator` is the deliberate exception — it bootstraps its own databases because applying migrations from scratch is what it tests.
