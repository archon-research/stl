...
- **Testing**:
    ...
    - **One container set per test *package*, never per test** — container startup, not the test, dominates integration-test CI time. Build `TestMain` from `testutil.NewIntegrationMain(m)` (`WithTimescaleDB`/`WithRedis`/`WithLocalStack`, plus `BeforeRun`/`AfterRun` for per-package setup) — it starts the set, publishes each container into a package var (`sharedDSN`, `sharedRedisAddr`, `sharedLocalStackCfg`) and folds the goroutine-leak check into the exit code. Under `internal/services/` that harness file is `setup_integration_test.go` (never `main_integration_test.go` — there is no `main.go` there). Isolate each test inside it: `testutil.SetupTestSchema(t, sharedDSN)` for Postgres (`SetupTestDatabase` when the SQL is schema-qualified, so `search_path` cannot isolate it), a `testutil.SanitizeTestName(t.Name())` prefix for Redis keys and SQS/SNS names, `testutil.S3TestBucketName(t, prefix)` for buckets. Anything a test counts (objects, messages, rows) needs its own bucket/queue/schema, not a shared one. `make shared-container-check` enforces this in `ci-checks`.
