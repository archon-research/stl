...
- **This service serves the frontend.** `make build-ui-static` builds `../ts/ui` and copies its `dist/` into `app/static`; `configure_static_hosting` (`app/main.py`) mounts it, so one process serves both the API and UI. See [../ts/CLAUDE.md](../ts/CLAUDE.md).
