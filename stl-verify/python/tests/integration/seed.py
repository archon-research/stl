...
async def insert_allocation_position(
    ...
    *,
    ...
    A reprocessing (``processing_version`` 1) is seeded by repeating a row with
    a fresh ``build_id`` and the SAME explicit ``created_at``: the assign trigger
...
ANCHORAGE_MULTI_PROXY_HEX = "ae" * 20  # multi-asset-type + multi-group fixtures
ANCHORAGE_OTHER_PROXY_HEX = "af" * 20  # a second prime with its own later cohort
_ANCHORAGE_VAULT_HEX = "ca" * 20
_ANCHORAGE_EMPTY_VAULT_HEX = "cb" * 20
_ANCHORAGE_MULTI_VAULT_HEX = "cd" * 20
_ANCHORAGE_OTHER_VAULT_HEX = "ce" * 20
_ANCHORAGE_DUMMY_TOKEN_HEX = "c1" * 20
_ANCHORAGE_TX = "c2" * 32
_ANCHORAGE_EMPTY_TX = "c3" * 32
_ANCHORAGE_MULTI_TX = "c4" * 32
_ANCHORAGE_OTHER_TX = "c5" * 32
...
# A second prime polled LATER than the main prime. If the per-prime scope on
# MAX(snapshot_time) is dropped, this later cohort becomes the global max and
# starves the main prime — the prime-isolation test's red signal.
...
async def _insert_anchorage_snapshot(
    ...
    *,
    ...
    build_id: int = 0,
    asset_type: str = "BTC",
    custody_type: str = "AnchorageCustody",
    asset_weighted_value: Decimal | None = None,
) -> None:
    """Insert one anchorage_package_snapshot row.

    ``exposure_value``/``package_value`` are package-level (repeated across a
    package's per-collateral-asset rows); ``asset_quantity`` and
    ``asset_weighted_value`` are per asset type. ``asset_weighted_value`` defaults
    to ``package_value`` (correct for a single-asset package). For a multi-asset
    package, insert one row per ``asset_type`` sharing ``package_id`` /
    ``snapshot_time`` and the same package-level values, with per-asset
    ``asset_quantity`` / ``asset_weighted_value``.
    ...
    """
    weighted = asset_weighted_value if asset_weighted_value is not None else package_value
    await conn.execute(
        """
...
async def seed_anchorage_custody(db_url: str) -> None:
    ...
    try:
        async with conn.transaction():
            ...
            await _seed_anchorage_multi_asset_prime(conn, token_id)
            await _seed_anchorage_other_prime(conn, token_id)
    finally:
        await conn.close()


async def _seed_anchorage_multi_asset_prime(conn: asyncpg.Connection, token_id: int) -> None:
    """A prime whose latest cohort spans two asset-type groups, one of them a
    multi-asset package — exercises multi-group output/ordering and the
    package-level double-count guard.

    Packages (all active, one snapshot_time):
      * PKG-B1  BTC only   — exposure 130, collateral 150, 10 BTC
      * PKG-E1  ETH only   — exposure  40, collateral  50,  5 ETH
      * PKG-MX  BTC + ETH  — one loan (exposure 30 / collateral 45) repeated on
                             both asset rows; BTC weighted 40 > ETH weighted 15,
                             so BTC is the package's dominant collateral.

    Correct (deduped) per-group sums:
      BTC → amount 160 (130 + 30), collateral 195, balance 13 (10 + 3)
      ETH → amount  40 (40 +  0), collateral  50, balance  7 ( 5 + 2)
    The package-level 30/45 of PKG-MX is attributed once (to BTC, its dominant
    ...
    await insert_allocation_position(
        ...
        token_id=token_id,
        prime_id=prime_id,
        proxy_hex=ANCHORAGE_MULTI_PROXY_HEX,
        balance=1,
        block=1000,
    ...
    await _insert_anchorage_snapshot(
        ...
        package_id="PKG-B1",
        active=True,
        exposure_value=Decimal("130"),
        package_value=Decimal("150"),
    ...
    await _insert_anchorage_snapshot(
        ...
        package_id="PKG-E1",
        active=True,
        exposure_value=Decimal("40"),
        package_value=Decimal("50"),
        asset_quantity=Decimal("5"),
        snapshot_time=ANCHORAGE_LATEST_SNAPSHOT,
        asset_type="ETH",
    )
    ...
    await _insert_anchorage_snapshot(
        ...
        package_id="PKG-MX",
        active=True,
        exposure_value=Decimal("30"),
        package_value=Decimal("45"),
        asset_quantity=Decimal("3"),
        snapshot_time=ANCHORAGE_LATEST_SNAPSHOT,
        asset_type="BTC",
    ...
    await _insert_anchorage_snapshot(
        ...
        snapshot_time=ANCHORAGE_LATEST_SNAPSHOT,
        asset_type="ETH",
        asset_weighted_value=Decimal("15"),
    )


async def _seed_anchorage_other_prime(conn: asyncpg.Connection, token_id: int) -> None:
    """A second prime with its own, later-polled BTC cohort (exposure 99).

    Later than the main prime's cohort on purpose: the per-prime scope on
    ``MAX(snapshot_time)`` must keep this prime's later poll from becoming the
    main prime's cohort.
    """
    prime_id = await conn.fetchval(
    ...
    await insert_allocation_position(
        ...
        token_id=token_id,
        prime_id=prime_id,
        proxy_hex=ANCHORAGE_OTHER_PROXY_HEX,
        balance=1,
        block=1000,
    ...
    await _insert_anchorage_snapshot(
        ...
        package_id="PKG-O1",
        active=True,
        exposure_value=Decimal("99"),
        package_value=Decimal("111"),
        asset_quantity=Decimal("9"),
...
# ---------------------------------------------------------------------------
# Receipt-position latest-row seed
#
# Exercises every case that distinguishes a correct latest-row rewrite of
# ``_RECEIPT_TOKEN_POSITIONS_SQL`` from a wrong one: several versions of one
# token, a reorg (higher block_version at the same block), a reprocessed row
# (processing_version > 0), a same-block log_index tie, a swept-to-zero token,
# a token with no receipt_token, two receipt tokens over one underlying, a
# receipt_token bound to a protocol on another chain, two token rows sharing one
# address across chains so that two (chain_id, token_id) pairs resolve to a
# single receipt_token, and one token_id carrying positions on two chains.
#
...
#
# Each token also gets _RTL_HISTORY_ROWS_PER_TOKEN older rows: enough joined rows
# that the pre-rewrite formulation sorts past a small work_mem and spills.
# ---------------------------------------------------------------------------
...
_RTL_HISTORY_ROWS_PER_TOKEN = 900
_RTL_DECISIVE_BLOCK = 9_000

# The reprocessed pair shares one created_at; see insert_allocation_position.
_RTL_REPROCESSED_AT = dt.datetime(2026, 5, 4, 12, 0, tzinfo=dt.UTC)
...
async def _rtl_seed_registry(conn: asyncpg.Connection) -> dict[str, int]:
    ...
    # receipt_token: two (chain_id, token_id) pairs resolve to a single
    # receipt_token id, which is why a (chain_id, token_id) dedup cannot replace
    # the dedup on rt.id.
    ...
    # One token row carrying positions on two ap.chain_id values, each resolving
    # to that chain's receipt_token. Nothing in the schema ties ap.chain_id to
    # token.chain_id (there is no FK on the pair), so grouping the latest-row
    # pre-filter on token_id alone would merge the two chains and lose the one
    # whose max block is lower.
...
async def _rtl_seed_positions(conn: asyncpg.Connection, *, prime_id: int, tokens: dict[str, int]) -> None:
    ...
    async def position(token_id: int, **kwargs) -> None:
        kwargs.setdefault("direction", "in")
        await insert_allocation_position(conn, token_id=token_id, prime_id=prime_id, proxy_hex=RTL_PROXY_HEX, **kwargs)
    ...
    await position(tokens["rtlReorg"], balance=Decimal("44"), block=_RTL_DECISIVE_BLOCK, tx=_rtl_tx(10), log_index=5)
    await position(
        tokens["rtlReorg"],
    ...
    # processing_version 1 and the corrected balance must win. The decoy at a
    # higher log_index (still version 0) makes an ordering that drops
    # processing_version pick 7 rather than merely tying.
    for build_id, balance in ((0, Decimal("77")), (1, RTL_LATEST_BALANCES["rtlReprocessed"])):
        await position(
            tokens["rtlReprocessed"],
            balance=balance,
            block=_RTL_DECISIVE_BLOCK,
            tx=_rtl_tx(20),
            created_at=_RTL_REPROCESSED_AT,
            build_id=build_id,
        )
    await position(
        tokens["rtlReprocessed"],
    ...
    await position(tokens["rtlLogIndex"], balance=Decimal("55"), block=_RTL_DECISIVE_BLOCK, tx=_rtl_tx(30), log_index=3)
    await position(
        tokens["rtlLogIndex"],
    ...
    for symbol, nonce in (("rtlSharedA", 40), ("rtlSharedB", 41)):
        await position(
            tokens[symbol], balance=RTL_LATEST_BALANCES[symbol], block=_RTL_DECISIVE_BLOCK, tx=_rtl_tx(nonce)
        )
    ...
    await position(tokens["rtlSwept"], balance=Decimal("500"), block=_RTL_DECISIVE_BLOCK, tx=_rtl_tx(50))
    await position(
        tokens["rtlSwept"],
        balance=Decimal("0"),
        block=_RTL_DECISIVE_BLOCK,
    ...
    await position(
        tokens["rtlMultiChain"],
        balance=RTL_LATEST_BALANCES["rtlMultiChain"],
        block=_RTL_DECISIVE_BLOCK,
        tx=_rtl_tx(90),
    )
    await position(
    ...
    await position(tokens["rtlAliasedTwin"], balance=Decimal("800"), block=_RTL_DECISIVE_BLOCK, tx=_rtl_tx(80))
    await position(
        tokens["rtlAliased"],
...
async def seed_receipt_position_latest_rows(db_url: str) -> None:
    """Seed the receipt-position latest-row scenarios into the given database."""
    conn = await asyncpg.connect(db_url)
    try:
        async with conn.transaction():
        ...
        # The no-spill assertion reads a real plan, so it depends on the planner
        # costing the aggregate off statistics rather than a fresh-table guess.
        await conn.execute("ANALYZE allocation_position")
    finally:
        await conn.close()
