...
async def _insert_morpho_vault(
    conn: asyncpg.Connection, protocol_id: int, chain_id: int, asset_token_id: int
) -> int:
    address = b"\xde\xad\xbe\xef" + b"\x00" * 16
    user_id = await insert_user(conn, address)
    vault_id = cast(
        int,
        await conn.fetchval(
            """
            ...
            RETURNING id
            """,
            chain_id, protocol_id, address, asset_token_id,
        ),
    )
    return vault_id
...
async def _insert_morpho_market(
    ...
    return cast(
        ...
        await conn.fetchval(
            ...
            """,
            chain_id, protocol_id, market_id, loan_token_id, collateral_token_id,
            b"\x00" * 20, b"\x00" * 20, lltv,
        ),
    )


async def _insert_market_position(
    conn: asyncpg.Connection, user_id: int, market_id: int
) -> None:
    import datetime
    await conn.execute(
        """
        ...
        """,
        user_id, market_id, datetime.datetime.now(tz=datetime.timezone.utc),
    )
...
async def _seed_data(db_url: str) -> None:
    ...
    try:
        ...
        user_id = cast(int, await conn.fetchval(
            'SELECT id FROM "user" WHERE address = $1 AND chain_id = 1', vault_address
        ))

        weth_market_id = await _insert_morpho_market(
            conn, protocol_id, chain_id=1,
            loan_token_id=usdc_id, collateral_token_id=weth_id,
            lltv=Decimal("0.86"), market_index=1,
        )
        cbbtc_market_id = await _insert_morpho_market(
            conn, protocol_id, chain_id=1,
            loan_token_id=usdc_id, collateral_token_id=cbbtc_id,
            lltv=Decimal("0.77"), market_index=2,
        )

        await _insert_market_position(conn, user_id, weth_market_id)
        await _insert_market_position(conn, user_id, cbbtc_market_id)

        await store_test_ids(conn, {
...
async def test_returns_lltv_as_threshold_and_lif_as_bonus(
    repository, test_ids: dict[str, int]
) -> None:
...
@pytest.mark.asyncio(loop_scope="module")
async def test_token_not_in_vault_markets_is_absent(
    repository, test_ids: dict[str, int]
) -> None:
    result = await repository.get_params(
        backed_asset_id=test_ids["vault_id"],
