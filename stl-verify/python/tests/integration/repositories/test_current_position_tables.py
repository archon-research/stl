...
async def _insert_offchain_price(
    conn: asyncpg.Connection, *, token_id: int, source_id: int, price: str, time_offset: str
) -> None:
    await conn.execute(
        """
        INSERT INTO offchain_token_price (token_id, source_id, "timestamp", price_usd)
        VALUES ($1, $2, NOW() + $3::text::interval, $4::numeric(30,18))
        """,
        token_id,
        source_id,
        time_offset,
...
@pytest.mark.asyncio(loop_scope="module")
async def test_offchain_price_cache_follows_observation_time_then_correction(conn: asyncpg.Connection) -> None:
    """One row per (token, source): a newer observation wins, an older one arriving late loses.

    A re-observation of the SAME instant (a correction) gets the next
    processing_version from the history's BEFORE trigger and overtakes the cached
    row on that last term — the two-term comparison 20260910_120100 documents.
    """
    token_id = await insert_token(conn, "CUROFFCHAIN", 18, b"\xb3" * 20)
    source_id = await conn.fetchval("SELECT id FROM offchain_price_source WHERE name = 'coingecko'")
    assert source_id is not None, "seed premise broken: the coingecko source is seeded by 20260204_120000"

    async def cached():
        row = await conn.fetchrow(
            "SELECT price_usd, snapshot_time, processing_version FROM offchain_token_price_current "
            "WHERE token_id = $1 AND source_id = $2",
            token_id,
            source_id,
        )
        assert row is not None
    ...
    await _insert_offchain_price(conn, token_id=token_id, source_id=source_id, price="1.5", time_offset="-2 hours")
    assert (await cached())["price_usd"] == Decimal("1.5")

    await _insert_offchain_price(conn, token_id=token_id, source_id=source_id, price="2.5", time_offset="-1 hours")
    newest = await cached()
    assert newest["price_usd"] == Decimal("2.5")
    assert newest["processing_version"] == 0

    await _insert_offchain_price(conn, token_id=token_id, source_id=source_id, price="0.5", time_offset="-3 hours")
    assert (await cached())["price_usd"] == Decimal("2.5")

    # Same observation instant as the cached row, from a later build: the
    # history's BEFORE trigger keys the version on (…, timestamp, build_id), so
    # this is the correction path and lands as processing_version 1.
    await conn.execute(
    ...
    corrected = await cached()
    assert corrected["price_usd"] == Decimal("2.75")
    assert corrected["processing_version"] == 1
    assert corrected["snapshot_time"] == newest["snapshot_time"]
