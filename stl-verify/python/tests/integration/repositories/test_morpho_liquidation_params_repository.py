...
# ---------------------------------------------------------------------------
# Shared helpers. The V1/V1.1 (get_params) and VaultV2 (get_params_v2) scenarios
# share ONE per-module DB, so their seed data must not collide: they share a single
# Morpho Blue protocol, distinct vault/adapter addresses, and disjoint market_ids
# (VaultV2 markets are offset by _V2_MARKET_ID_OFFSET below to avoid the V1 markets).
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# V1 / V1.1 (get_params) seed helpers
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# V2 (VaultV2 adapter-graph) seed helpers + constants
# ---------------------------------------------------------------------------
...
# VaultV2 markets live in a market_id namespace disjoint from the V1 markets (which
# use raw indices 1..2) so the two scenarios can share one DB without a unique clash.
_V2_MARKET_ID_OFFSET = 100
...
async def _insert_market(
    ...
    return cast(
        ...
        """,
            protocol_id,
            bytes([_V2_MARKET_ID_OFFSET + index]) + b"\x00" * 31,
            loan_token_id,
            collateral_token_id,
            b"\x00" * 20,
            b"\x00" * 20,
...
# ---------------------------------------------------------------------------
# Combined seed. One shared DB holds:
#   V1: a MetaMorpho V1 vault whose OWN market positions supply WETH@0.86 + cbBTC@0.77.
#   V2 (all lltv WAD): the V2 vault's liquidatable collateral is reachable only through
#     its ACTIVE adapters —
...
#   plus one single-adapter vault per has_unwalkable_adapter_value branch.
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# V1 / V1.1 tests (get_params: vault address is itself the Blue-market user)
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# V2 (VaultV2) tests: get_params_v2 resolves collateral through the adapter graph.
# ---------------------------------------------------------------------------
...
async def test_resolves_lltv_across_adapter_graph(repository, test_ids: dict[str, int]) -> None:
    ...
    result = await repository.get_params_v2(
...
async def test_empty_token_ids_returns_empty(repository, test_ids: dict[str, int]) -> None:
    assert await repository.get_params_v2(backed_asset_id=test_ids["v2_vault"], token_ids=[]) == {}
