...
async def prime_ids(module_db) -> dict[str, int]:
    """The prime each seeded proxy belongs to; custody is keyed on the prime."""
    conn = await asyncpg.connect(module_db["db_url"])
    try:
        rows = await conn.fetch("SELECT encode(proxy_address, 'hex') AS hex, prime_id FROM prime_proxy")
        return {row["hex"]: row["prime_id"] for row in rows}
    finally:
        await conn.close()
...
@pytest.mark.asyncio
async def test_collapses_current_cohort_into_one_btc_custody_row(repo, prime_ids) -> None:
    ...
    """
    holdings = await repo.list_anchorage_custody_holdings(prime_ids[ANCHORAGE_CUSTODY_PROXY_HEX])
...
@pytest.mark.asyncio
async def test_cohort_filter_excludes_stale_closed_packages(repo, prime_ids) -> None:
...
@pytest.mark.asyncio
async def test_processing_version_correction_supersedes_original(repo, prime_ids) -> None:
...
@pytest.mark.asyncio
async def test_active_predicate_excludes_inactive_package_in_cohort(repo, prime_ids) -> None:
...
@pytest.mark.asyncio
async def test_returns_empty_for_prime_with_no_custody_snapshots(repo, prime_ids) -> None:
    """A known prime with no Anchorage rows yields an empty list, not an error."""
    holdings = await repo.list_anchorage_custody_holdings(prime_ids[ANCHORAGE_EMPTY_PROXY_HEX])

    assert holdings == []


@pytest.mark.asyncio
async def test_multiple_asset_type_groups_ordered_by_amount_desc(repo, prime_ids) -> None:
    ...
    """
    holdings = await repo.list_anchorage_custody_holdings(prime_ids[ANCHORAGE_MULTI_PROXY_HEX])
...
@pytest.mark.asyncio
async def test_multi_asset_package_loan_not_double_counted_across_groups(repo, prime_ids) -> None:
...
@pytest.mark.asyncio
async def test_cohort_is_scoped_per_prime(repo, prime_ids) -> None:
    ...
    """
    main = await repo.list_anchorage_custody_holdings(prime_ids[ANCHORAGE_CUSTODY_PROXY_HEX])
    other = await repo.list_anchorage_custody_holdings(prime_ids[ANCHORAGE_OTHER_PROXY_HEX])
