"""Session DB + per-test transaction rollback fixtures.
...
def get_session_metadata() -> MetaData:
    """Return reflected MetaData populated by the ``session_engine`` fixture.

    Raises if called before ``session_engine`` has run.
    """
...
async def session_engine(pg_container: PostgresContainer) -> AsyncIterator[AsyncEngine]:
    ...
    Reflects ``MetaData`` once; cached for seed helpers via ``get_session_metadata()``.
    """
