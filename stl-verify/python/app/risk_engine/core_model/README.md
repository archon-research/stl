...
The financial model logic (ARMA-GARCH calibration, copula simulation, liquidation mechanics) is **mathematically unchanged** with three exceptions — the `slippage_calculator_cum` partial-tick fix, the innovation-dispatch fix, and the per-token jump-params fix (each has its own row below) — all of which correct defects rather than change the model's design. The other modifications were made for service integration:
...
| Each token simulates with its own calibrated jump params (`runner.py` stores `jump_params` per token) | Upstream calibrated the compound-Poisson jump process per token but reassigned one shared `JUMP_PARAMS` variable each iteration and never populated the per-token entry `forecaster.py` reads, so `simulate_prices` applied the **last** token's jumps to every token (audit C-08). `JUMPS=true` is the production default, so this affected every live multi-collateral market. The calibration itself is unchanged — the fix only delivers each token's params to its own simulation. Single-token markets are unaffected (last = only). This moves multi-token CRRs; see [VEC-768](https://linear.app/archontech/issue/VEC-768/core-model-per-token-jump-params-never-reach-the-simulator-c-08) and its PR for the before/after runs. |
| `brownian_bridge_hourly` raises on non-finite hourly returns | Upstream printed a warning and substituted zeros, so degenerate daily returns/volatility produced flat price paths persisted as an apparently valid CRR (audit C-09). Only reachable with `HOURLY_CONV=true`, off by default. |
| `load_params` validates parameter types | A JSON or dict override of the wrong type reached simulation unchecked — `"WORST_CASE": "false"` is a non-empty string and activated the truthy branch (audit T-01). Types come from the schema's declared `type` or the default's Python type; the min/max/choices stay advisory, matching upstream's convention. |
| `Backtester` computes the Kupiec and Christoffersen statistics in log space | Both selection gates were vacuous (audit C-06/C-07). Kupiec special-cased an observed exceedance rate of exactly 0 or 1 to `LR=0, p=1` (dodging `log(0)`), so the two most miscalibrated outcomes — an all-hit or no-hit window — passed unconditional coverage unconditionally; the correct statistic drops the zero-count log-likelihood terms (`0·log 0 = 0`) and is huge (no-hit over 1280 days at `alpha=0.05`: `LR = −2·1280·ln 0.95 ≈ 131`, `p ≈ 10⁻³⁰`). Christoffersen multiplied one sub-unit probability per observation into raw likelihoods and clipped both to the same `1e-10` floor; past ~440 observations (`0.95⁴⁴⁰ ≈ 1e-10`) both clip, the ratio is 1 and `LR_ind = 0, p = 1`, so with ~1280 rolling hits the independence test passed everything, including a perfect 64-day crash cluster. Both now accumulate log-likelihoods with zero-count terms dropped (each probability is the MLE of the counts it multiplies, so it is strictly interior wherever its count is nonzero). Conditional coverage (`LR_cc = LR_ind + LR_pof`) inherits both fixes. This can change which GARCH model wins selection, so CRR movement is indirect; see [VEC-766](https://linear.app/archontech/issue/VEC-766/core-model-backtester-accepts-miscalibrated-models-c-06-c-07-wrong) and its PR for the before/after runs (no movement on the reference markets). |
| `hit_backtest` requires its `use_log_returns`/`alpha` arguments | `total_fitter` called `hit_backtest` on its defaults instead of plumbing `use_log_returns`/`backtest_alpha` through (formerly Known Issue #3). The defaults happened to match production (`USE_LOG_RETURNS=True`), so this was latent, but any non-default run would have selected models on one return type and backtested on the other. The arguments are now required (`alpha` as the VaR tail probability, same convention as the tests) and passed explicitly. See [VEC-766](https://linear.app/archontech/issue/VEC-766/core-model-backtester-accepts-miscalibrated-models-c-06-c-07-wrong). |
...
2. Pass rolling VaR backtests: **Kupiec** (unconditional coverage) and **Christoffersen** (conditional coverage / independence) at a fixed `backtest_alpha = 0.05` tail, independent of `PERC` (rationale: the **Calibration choices** table in `documentation/core_model_methodology.md`)
...
Kupiec tests unconditional coverage (the hit rate matches `backtest_alpha`) and Christoffersen tests independence (hits do not cluster); both statistics are accumulated in log space so boundary hit rates and long samples are scored correctly (fixed in VEC-766 — see the deviations table).
