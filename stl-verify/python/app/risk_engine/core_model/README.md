...
| `Backtester` computes the Kupiec and Christoffersen statistics in log space, and `hit_backtest` requires its `use_log_returns`/`alpha` arguments | Three defects made the GARCH selection gates vacuous (audit C-06/C-07, Known Issue #3). Kupiec special-cased an observed exceedance rate of exactly 0 or 1 to `LR=0, p=1` (dodging `log(0)`), so the two most miscalibrated outcomes — an all-hit or no-hit window — passed unconditional coverage unconditionally; the correct statistic drops the zero-count log-likelihood terms (`0·log 0 = 0`) and is huge (no-hit over 1280 days at `alpha=0.05`: `LR = −2·1280·ln 0.95 ≈ 131`, `p ≈ 10⁻³⁰`). Christoffersen multiplied one sub-unit probability per observation into raw likelihoods and clipped both to the same `1e-10` floor; past ~440 observations (`0.95⁴⁴⁰ ≈ 1e-10`) both clip, the ratio is 1 and `LR_ind = 0, p = 1`, so with ~1280 rolling hits the independence test passed everything, including a perfect 64-day crash cluster. Both now accumulate log-likelihoods with zero-count terms dropped, which also needs no probability clipping (each probability is the MLE of the counts it multiplies). Conditional coverage (`LR_cc = LR_ind + LR_pof`) inherits both fixes. Finally, `total_fitter` called `hit_backtest` on its defaults instead of plumbing `use_log_returns`/`backtest_alpha` through; the defaults happened to match production (`USE_LOG_RETURNS=True`), so this was latent, but any non-default run would have selected models on one return type and backtested on the other. The arguments are now required (`alpha` as the VaR tail probability, same convention as the tests) and passed explicitly. This changes which GARCH model wins selection, so CRR movement is indirect; see [VEC-766](https://linear.app/archontech/issue/VEC-766/core-model-backtester-accepts-miscalibrated-models-c-06-c-07-wrong) and its PR for the before/after runs. |
