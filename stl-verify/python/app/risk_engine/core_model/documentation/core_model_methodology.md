...
Accepted candidates are then subject to a **rolling 1-step-ahead VaR backtest**. The model is trained on a window of `TRAIN_SIZE` days and the 1-day-ahead VaR is computed at a fixed `backtest_alpha = 0.05` tail, independent of `PERC`. This doc previously said `backtest_alpha = 1 - PERC` — an upstream documentation error that no code ever implemented: upstream hardcoded the same 0.05 at both ends (`total_fitter` default, `hit_backtest`'s 0.95 confidence level) and never passed anything else. The fixed 5% tail also gives roughly twice the expected hits (~64 over 4 years) of `1 − PERC = 0.025`, which is what gives the Kupiec and Christoffersen tests their power. The window then rolls forward by 1 day, producing approximately (`N_history` − `TRAIN_SIZE`) non-overlapping hit observations — roughly 1 280 over a 4-year history with a 180-day training window. Two statistical tests are applied to the resulting hit sequence:
