...
class ChainRiskCapital:
    """One chain's contribution to its prime's aggregated figures.

    Carried so a consumer can audit the prime-level sum, and so a chain
    contributing nothing is visibly present rather than absent.

    The figures are ``None`` — not ``0`` — for a chain no allocation tracker
    serves. The prime has no ``allocation_position`` rows there at all, so a zero
    would claim it holds nothing when the truth is that STL does not know: the
    two must not read alike, because the difference moves the prime's
...
class PrimeRiskCapital:
    ...
    exposure the model could price. ``encumbrance_ratio`` is ``None`` when Total
    Risk Capital is absent or zero.

    The aggregate covers the prime's ALM proxies on chains an allocation tracker
    serves; the rest contribute nothing and are named in
