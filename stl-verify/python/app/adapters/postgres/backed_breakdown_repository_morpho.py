# ruff: noqa: E501
"""Backed-breakdown repository for Morpho vaults (MetaMorpho V1/V1.1 and VaultV2).

One repository serves every ``vault_version``: ``get_backed_breakdown`` walks the
direct Morpho-Blue allocation of a MetaMorpho V1/V1.1 vault, and
``get_backed_breakdown_v2`` walks the adapter graph of a Morpho VaultV2. Both resolve a
vault the same way (``resolve_vault``) and map a priced row to a
...
class MorphoVaultRef:
    ...
    Morpho VaultV2 (adapter-based). Returned by ``resolve_vault`` so the reader can
    dispatch on version without embedding SQL.
    """
...
# Shared by the V1 and V2 walks below.
...
class MorphoBackedBreakdownRepository:
    """Postgres backed-breakdown repository for Morpho vaults (V1/V1.1 and VaultV2).

    ``get_backed_breakdown`` walks a MetaMorpho V1/V1.1 vault's direct Morpho-Blue
    allocation; ``get_backed_breakdown_v2`` walks a Morpho VaultV2's adapter graph.
    Both map rows through the shared ``to_contribution`` (identical USD-scaling), and
    ``resolve_vault`` returns the ``vault_version`` the reader dispatches on.
    """
    ...
    async def resolve_vault(self, address: bytes, chain_id: int) -> MorphoVaultRef | None:
        ...
        async with self._engine.connect() as connection:
            result = await connection.execute(text(_VAULT_RESOLVE_SQL), {"addr": address, "chain_id": chain_id})
            row = result.fetchone()
    ...
    async def get_backed_breakdown(self, backed_asset_id: int) -> BackedBreakdown:
        """Execute the MetaMorpho V1/V1.1 vault backed breakdown query and return domain objects."""
        async with self._engine.connect() as connection:
    ...
    async def get_backed_breakdown_v2(self, backed_asset_id: int) -> BackedBreakdown:
        """Execute the Morpho VaultV2 adapter-graph backed-breakdown walk and return domain objects.

        Rows map to ``CollateralContribution`` via the shared ``to_contribution`` (same
        USD-scaling as the V1 walk): backing_value = loan-token amount × loan-token
