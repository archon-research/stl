...
# V1/V1.1: the vault address is itself the Morpho-Blue user, so its collateral markets
# are the markets its own positions supply.
_PARAMS_SQL = """
WITH vault_user AS (
...
# V2 (VaultV2): collateral tokens' MIN(lltv) reachable through the vault's ACTIVE
# adapters. A VaultV2 never supplies to Morpho Blue under its own address, so the walk
# keys strictly on adapter users and deliberately ignores any morpho_market_position
# attributed to the vault address itself (e.g. vault 760 wethpv's single dust row is a
# misattributed artifact, not a real allocation).
_V2_PARAMS_SQL = """
WITH v AS (
...
class MorphoLiquidationParamsRepository:
    """Liquidation-params adapter for Morpho Blue vaults (V1/V1.1 and VaultV2).

    liquidation_threshold = lltv (same concept, same value).
    liquidation_bonus     = LIF computed deterministically from lltv.

    ``get_params`` resolves a MetaMorpho V1/V1.1 vault through the vault address's own
    market positions; ``get_params_v2`` resolves a VaultV2 through its active adapter
    graph. Two composition probes let the reader distinguish a deployed-but-unindexed
    v3 vault (degrade to ``adapter_data_missing``) from a genuinely idle one (a real
    ...
    async def get_params_v2(self, backed_asset_id: int, token_ids: list[int]) -> dict[int, LiquidationParams]:
        """Resolve VaultV2 liquidation params through the active adapter graph."""
        if not token_ids:
            return {}
