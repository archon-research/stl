...
// Each subtest names its own key, so one migrated schema carries all of them (AGENTS.md, "Share setup,
// don't repeat it") without their positions or run records colliding.
func windowFixture(t *testing.T, ctx context.Context, pool *pgxpool.Pool, key string) string {
	t.Helper()
	view := "position_win_" + key
	if _, err := pool.Exec(ctx, fmt.Sprintf(`
		CREATE TABLE wsrc_%[1]s (ts timestamptz NOT NULL, holder text NOT NULL, ik text NOT NULL,
		                   qty numeric NOT NULL, bn bigint NOT NULL);
		INSERT INTO wsrc_%[1]s SELECT date_trunc('day', now()) + interval '12 hours' - (g||' days')::interval,
		       'aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa', 'WIN_%[1]s', 100, 5000-g FROM generate_series(1,40) g;
		INSERT INTO wsrc_%[1]s VALUES (date_trunc('day', now()) + interval '12 hours', 'aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa', 'WIN_%[1]s', 0, 9000);
		CREATE VIEW %[2]s AS
		...
		FROM wsrc_%[1]s ORDER BY holder, ik, bn, ts`, key, view)); err != nil {
		t.Fatalf("build the window fixture for %s: %v", key, err)
	}
	return view
}

func materializeWindow(t *testing.T, ctx context.Context, pool *pgxpool.Pool, view string, window any) int64 {
	t.Helper()
	var n int64
	if err := pool.QueryRow(ctx,
		`SELECT materialize_position_projection(('public.'||$1)::regclass, 0, NULL, $2)`, view, window).Scan(&n); err != nil {
		t.Fatalf("materialize %s with window %v: %v", view, window, err)
	}
	return n
}

func storedWindowRows(t *testing.T, ctx context.Context, pool *pgxpool.Pool, key string) int {
	t.Helper()
	var n int
	if err := pool.QueryRow(ctx,
		`SELECT count(*) FROM position_state WHERE instrument_key = $1`, "WIN_"+key).Scan(&n); err != nil {
		t.Fatalf("count stored rows for %s: %v", key, err)
	}
	return n
}

// One migrated database for the whole window suite: the subtests touch disjoint views and instrument
// keys, so they share the schema the way position_state_integration_test.go does.
func TestProjectionWindow(t *testing.T) {
	...
	for _, tc := range []struct {
		name string
		run  func(*testing.T, context.Context, *pgxpool.Pool)
	}{
		{"NullIsUnbounded", pwTestNullIsUnbounded},
		{"BoundedFromColdCannotDiscoverHistory", pwTestBoundedFromColdCannotDiscoverHistory},
		{"IsStampedOnTheRunRecord", pwTestIsStampedOnTheRunRecord},
		{"BoundedAfterBootstrapClosesAndIsIdempotent", pwTestBoundedAfterBootstrapClosesAndIsIdempotent},
		{"IsInterpolatedAsALiteral", pwTestIsInterpolatedAsALiteral},
		{"RefusesANonPositiveInterval", pwTestRefusesANonPositiveInterval},
		{"LeavesExactlyOneSignature", pwTestLeavesExactlyOneSignature},
	} {
		t.Run(tc.name, func(t *testing.T) { tc.run(t, ctx, pool) })
	}
}

// A NULL window is unbounded, which is what bootstrap needs: every observation is discovered.
func pwTestNullIsUnbounded(t *testing.T, ctx context.Context, pool *pgxpool.Pool) {
	view := windowFixture(t, ctx, pool, "null")
	if appended := materializeWindow(t, ctx, pool, view, nil); appended != 41 {
		t.Errorf("unbounded run appended %d, want 41", appended)
	}
	if rows := storedWindowRows(t, ctx, pool, "null"); rows != 41 {
...
func pwTestBoundedFromColdCannotDiscoverHistory(t *testing.T, ctx context.Context, pool *pgxpool.Pool) {
	view := windowFixture(t, ctx, pool, "cold")
	if appended := materializeWindow(t, ctx, pool, view, "36 hours"); appended != 2 {
		t.Fatalf("bounded cold run appended %d, want exactly 2 (the g=1 row and the closing zero)", appended)
	}
	if rows := storedWindowRows(t, ctx, pool, "cold"); rows != 2 {
...
func pwTestIsStampedOnTheRunRecord(t *testing.T, ctx context.Context, pool *pgxpool.Pool) {
	view := windowFixture(t, ctx, pool, "stamp")
	materializeWindow(t, ctx, pool, view, nil)
	materializeWindow(t, ctx, pool, view, "2 days")
	var got string
	if err := pool.QueryRow(ctx, `
		SELECT string_agg(CASE WHEN window_interval IS NULL THEN 'NULL'
		  ...
		  FROM position_projection_run WHERE projection = 'public.'||$1`, view).Scan(&got); err != nil {
...
func pwTestBoundedAfterBootstrapClosesAndIsIdempotent(t *testing.T, ctx context.Context, pool *pgxpool.Pool) {
	view := windowFixture(t, ctx, pool, "boot")
	if appended := materializeWindow(t, ctx, pool, view, nil); appended != 41 {
		t.Fatalf("bootstrap appended %d, want 41", appended)
	}
	if appended := materializeWindow(t, ctx, pool, view, "2 days"); appended != 0 {
		t.Errorf("bounded re-run appended %d, want 0", appended)
	}
	var closing string
	if err := pool.QueryRow(ctx, `
		SELECT quantity::text FROM position_state WHERE instrument_key = 'WIN_boot'
	...
	if rows := storedWindowRows(t, ctx, pool, "boot"); rows != 41 {
...
func pwTestIsInterpolatedAsALiteral(t *testing.T, ctx context.Context, pool *pgxpool.Pool) {
	var src string
	if err := pool.QueryRow(ctx, `
...
func pwTestRefusesANonPositiveInterval(t *testing.T, ctx context.Context, pool *pgxpool.Pool) {
	view := windowFixture(t, ctx, pool, "guard")
	for _, w := range []string{"0", "-1 days", "infinity"} {
		var n int64
		err := pool.QueryRow(ctx,
			`SELECT materialize_position_projection(('public.'||$1)::regclass, 0, NULL, $2)`, view, w).Scan(&n)
...
func pwTestLeavesExactlyOneSignature(t *testing.T, ctx context.Context, pool *pgxpool.Pool) {
	view := windowFixture(t, ctx, pool, "sig")
	var signatures int
	if err := pool.QueryRow(ctx, `
	...
	var n int64
	if err := pool.QueryRow(ctx,
		`SELECT materialize_position_projection(('public.'||$1)::regclass, 0, NULL)`, view).Scan(&n); err != nil {
