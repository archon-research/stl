...
// position_morpho_market_since is the view's SELECT with the bound inside the morpho_market_position scan
// (VEC-566), plus one thing the view does not need: the direction carry cannot see a non-zero observation
// before the bound, so a directionless net-zero row takes the direction of the position's latest STORED
// observation. Pinned: equal to the view at -infinity; equal to the view over the window once history is
// stored; NULL without that history, which is why bootstrap is unbounded; and a tail bound prunes.
func TestMorphoMarketBoundedSourceMatchesTheViewAndPrunes(t *testing.T) {
	ctx, pool, _ := materializeMorphoMarketFixture(t)

	drift := func(bounded, view string) int {
		var n int
		if err := pool.QueryRow(ctx, `
			SELECT count(*) FROM (
			  (SELECT * FROM `+view+` EXCEPT ALL SELECT * FROM `+bounded+`)
			  UNION ALL
			  (SELECT * FROM `+bounded+` EXCEPT ALL SELECT * FROM `+view+`)) d`).Scan(&n); err != nil {
			t.Fatal(err)
		}
		return n
	}
	if n := drift(`position_morpho_market_since('-infinity')`, `position_morpho_market`); n != 0 {
		t.Fatalf("%d rows differ between the view and its bounded source at -infinity", n)
	}
	var after int
	if err := pool.QueryRow(ctx, `SELECT count(*) FROM position_morpho_market_since('infinity')`).Scan(&after); err != nil {
		t.Fatal(err)
	}
	if after != 0 {
	...
	// A borrower who repays across the bound: BORROW on day 1, net zero on day 10. Stored by an
	// unbounded run first, as bootstrap is.
	if _, err := pool.Exec(ctx, `
		INSERT INTO "user" (chain_id, address) VALUES (1, '\xf9f9f9f9f9f9f9f9f9f9f9f9f9f9f9f9f9f9f9f9');
		INSERT INTO morpho_market_position (user_id, morpho_market_id, block_number, block_version, timestamp, supply_shares, borrow_shares, collateral, supply_assets, borrow_assets)
		SELECT u.id, m.id, 90001, 0, '2026-03-01T12:00:00Z', 0, 0, 0, 0, 50
		FROM "user" u, morpho_market m WHERE u.address = '\xf9f9f9f9f9f9f9f9f9f9f9f9f9f9f9f9f9f9f9f9' AND m.market_id = '\x1234';
		INSERT INTO morpho_market_position (user_id, morpho_market_id, block_number, block_version, timestamp, supply_shares, borrow_shares, collateral, supply_assets, borrow_assets)
		SELECT u.id, m.id, 90000 + d, 0, '2026-03-01T12:00:00Z'::timestamptz + (d - 1) * interval '1 day', 0, 0, 0, 0, 0
		FROM generate_series(2, 10) d, "user" u, morpho_market m WHERE u.address = '\xf9f9f9f9f9f9f9f9f9f9f9f9f9f9f9f9f9f9f9f9' AND m.market_id = '\x1234'`); err != nil {
		t.Fatal(err)
	}
	const window = `'2026-03-05T00:00:00Z'::timestamptz`
	var stored string
	if err := pool.QueryRow(ctx, `SELECT coalesce(deal_type, 'NULL') FROM position_morpho_market_since(`+window+`) WHERE holder_id = 'f9f9f9f9f9f9f9f9f9f9f9f9f9f9f9f9f9f9f9f9' AND block_number = 90010`).Scan(&stored); err != nil {
		t.Fatal(err)
	}
	if stored != "NULL" {
		t.Errorf("before any run is stored, the windowed close has direction %s; the fallback found history that does not exist", stored)
	}
	...
	if stored != "BORROW" {
		t.Errorf("the windowed close of a repaid borrow has direction %s, want BORROW from the stored history", stored)
	}
	if n := drift(`position_morpho_market_since(`+window+`)`, `position_morpho_market WHERE block_timestamp > `+window); n != 0 {
		t.Errorf("%d rows differ between the view over the window and the bounded source, with history stored", n)
	}

	all := marketChunkScans(t, ctx, pool, `SELECT * FROM position_morpho_market`)
	outside := marketChunkScans(t, ctx, pool, `SELECT * FROM position_morpho_market WHERE block_timestamp > `+window)
	inside := marketChunkScans(t, ctx, pool, `SELECT * FROM position_morpho_market_since(`+window+`)`)
	...
	if inside > 7 {
		t.Errorf("a bound inside the source planned %d chunk scans, want at most the 6 days after the bound plus one", inside)
	}
}

// marketChunkScans counts the DISTINCT morpho_market_position chunks a statement's plan reads: EXPLAIN
// names each as _hyper_N_M_chunk, a chunk can appear on more than one plan line, and the bounded source
// also probes position_state's chunks for the stored direction, which are not the source being pruned.
func marketChunkScans(t *testing.T, ctx context.Context, pool *pgxpool.Pool, sql string) int {
	t.Helper()
	source := map[string]bool{}
	names, err := pool.Query(ctx, `SELECT chunk_name FROM timescaledb_information.chunks WHERE hypertable_name = 'morpho_market_position'`)
	if err != nil {
		t.Fatal(err)
	}
	for names.Next() {
