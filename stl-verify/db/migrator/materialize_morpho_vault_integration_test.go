...
// position_morpho_vault_since is the view's SELECT with the bound inside the morpho_vault_position scan
// (VEC-566). It emits exactly the view's rows when the bound precedes everything, and a tail bound opens
// only the tail's chunks, where the same bound outside the view -- above its DISTINCT ON -- opens every
// chunk, the control that says the placement matters.
func TestMorphoVaultBoundedSourceMatchesTheViewAndPrunes(t *testing.T) {
	ctx, pool, _ := seedMorphoVault(t)

	var drift, rows int
	if err := pool.QueryRow(ctx, `
		SELECT (SELECT count(*) FROM (
		          (TABLE position_morpho_vault EXCEPT ALL SELECT * FROM position_morpho_vault_since('-infinity'))
		          UNION ALL
		          (SELECT * FROM position_morpho_vault_since('-infinity') EXCEPT ALL TABLE position_morpho_vault)) d),
		       (SELECT count(*) FROM position_morpho_vault)`).Scan(&drift, &rows); err != nil {
		t.Fatal(err)
	}
	if rows == 0 || drift != 0 {
	...
	var after int
	if err := pool.QueryRow(ctx, `SELECT count(*) FROM position_morpho_vault_since('infinity')`).Scan(&after); err != nil {
		t.Fatal(err)
	}
	if after != 0 {
	...
	// Ten more days of history on one deposit, one observation per day, on the 1-day-chunked source.
	if _, err := pool.Exec(ctx, `
		INSERT INTO morpho_vault_position (user_id, morpho_vault_id, block_number, block_version, timestamp, shares, assets)
		SELECT s.user_id, s.morpho_vault_id, 90000 + d, 0, '2026-03-01T12:00:00Z'::timestamptz + (d - 1) * interval '1 day', d, d
		FROM generate_series(1, 10) d, (SELECT user_id, morpho_vault_id FROM morpho_vault_position ORDER BY 1, 2 LIMIT 1) s`); err != nil {
		t.Fatal(err)
	}
	all := chunkScans(t, ctx, pool, `SELECT * FROM position_morpho_vault`)
	outside := chunkScans(t, ctx, pool, `SELECT * FROM position_morpho_vault WHERE block_timestamp > '2026-03-09T00:00:00Z'::timestamptz`)
	inside := chunkScans(t, ctx, pool, `SELECT * FROM position_morpho_vault_since('2026-03-09T00:00:00Z'::timestamptz)`)
...
// chunkScans counts the hypertable chunks a statement's plan reads: EXPLAIN names each as _hyper_N_M_chunk.
func chunkScans(t *testing.T, ctx context.Context, pool *pgxpool.Pool, sql string) int {
	t.Helper()
