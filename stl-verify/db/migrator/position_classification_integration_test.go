...
// deal_type (FK) and a non-LONG/SHORT direction (CHECK). ref_deal_type is seeded by the
...
func TestPositionClassification(t *testing.T) {
	...
	if _, err := pool.Exec(ctx,
		`INSERT INTO position_classification (position_id, deal_type, direction)
		 VALUES (sha256('valid'::bytea), 'LOAN', 'LONG')`); err != nil {
		t.Fatalf("valid classification insert: %v", err)
	}

	// FK: an unknown deal_type is rejected.
	if _, err := pool.Exec(ctx,
		`INSERT INTO position_classification (position_id, deal_type)
		 VALUES (sha256('fk'::bytea), 'NOT_A_DEAL_TYPE')`); err == nil {
		t.Error("expected a foreign-key violation for an unknown deal_type")
	}
