...
func TestMaterializePrimeAllocationForwardsTheWriterRun(t *testing.T) {
	ctx, pool, _ := seedPrimeAllocation(t)
	if _, err := pool.Exec(ctx, `SELECT materialize_prime_allocation(7, 9182)`); err != nil {
		t.Fatalf("materialize_prime_allocation with a run: %v", err)
	}
	var runID *int64
	var buildID int
	if err := pool.QueryRow(ctx, `
	...
	var args []string
	if err := pool.QueryRow(ctx, `
		SELECT proargnames::text[] FROM pg_proc WHERE proname = 'materialize_prime_allocation'`).Scan(&args); err != nil {
	...
	for _, want := range []string{"p_build_id", "p_run_id"} {
		...
		if !found {
			t.Errorf("materialize_prime_allocation declares %v, missing %s -- the runner calls it by name", args, want)
		}
	}
}
