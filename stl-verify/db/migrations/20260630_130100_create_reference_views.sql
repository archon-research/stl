...
COMMENT ON VIEW security_classification IS 'Flattened security classification catalog: asset_class -> security_type -> security_subtype. security_subtype is NULL where the type does not subdivide; leaf_code waterfalls to the most-specific available code (subtype else type) and leaf_level marks which (SUBTYPE/TYPE). Read-only convenience over the reference tables.';
