...
-- identity AGENTS.md prescribes for assets with no token row (token_id stays NULL).
...
-- (prefix: ap) with plan_cache_mode pinned (VEC-541).
...
ALTER TABLE asset_price SET (
    timescaledb.compress,
    timescaledb.compress_segmentby = 'asset_id',
    timescaledb.compress_orderby = 'timestamp DESC, processing_version DESC'
);

SELECT add_compression_policy('asset_price', INTERVAL '60 days', if_not_exists => TRUE);

-- Covering index for the version rule's per-key lookups, matching
-- idx_offchain_token_price_pv_lookup (20260424_120000).
CREATE INDEX IF NOT EXISTS idx_asset_price_pv_lookup
    ON asset_price (asset_id, source_id, timestamp, processing_version DESC);

DO $$ BEGIN
    PERFORM add_tiering_policy('asset_price', INTERVAL '1 year', if_not_exists => TRUE);
EXCEPTION WHEN undefined_function THEN
    RAISE NOTICE 'add_tiering_policy not available, skipping tiering for asset_price';
END $$;
...
CREATE OR REPLACE FUNCTION next_processing_version_asset_price(
    p_asset_id  BIGINT,
...
BEGIN
    ...
    PERFORM pg_advisory_xact_lock(hashtextextended(
        format('ap|%s|%s|%s', p_asset_id, p_source_id, EXTRACT(epoch FROM p_timestamp)), 0));
...
COMMENT ON FUNCTION next_processing_version_asset_price(BIGINT, SMALLINT, TIMESTAMPTZ, INT) IS
  'Returns the processing_version an asset_price row at (asset_id, source_id, timestamp) must carry for build_id: the version that build already wrote there, else MAX+1. Takes the row''s advisory lock (ADR-0002 §3) and holds it for the transaction. Call it in the INSERT''s VALUES list: on a columnstored chunk TimescaleDB resolves ON CONFLICT before row triggers fire, so a version left to the trigger reaches the arbiter as DEFAULT 0 and the correction row is silently discarded.';
...
CREATE OR REPLACE FUNCTION assign_processing_version_asset_price()
RETURNS TRIGGER
SET plan_cache_mode = 'force_custom_plan'
AS $$
BEGIN
    NEW.processing_version := next_processing_version_asset_price(
...
CREATE TRIGGER trigger_assign_processing_version
    BEFORE INSERT ON asset_price
    FOR EACH ROW
EXECUTE FUNCTION assign_processing_version_asset_price();

COMMENT ON TABLE asset_price IS
...
COMMENT ON COLUMN asset_price.asset_id IS
...
COMMENT ON COLUMN asset_price.source_id IS
...
REVOKE UPDATE, DELETE ON asset_price FROM stl_readwrite;

-- tokenless declares, rather than infers, that an asset's prices belong in
-- asset_price. token_id NULL alone cannot carry that meaning: the
...
ALTER TABLE offchain_price_asset
    ADD COLUMN IF NOT EXISTS tokenless BOOLEAN NOT NULL DEFAULT FALSE;
ALTER TABLE offchain_price_asset
    ADD CONSTRAINT offchain_price_asset_identity_check
    CHECK (NOT (tokenless AND token_id IS NOT NULL));
COMMENT ON COLUMN offchain_price_asset.tokenless IS
  'Roles: Configuration. TRUE = the asset deliberately has no token row (native of a non-indexed ecosystem: XRP, HYPE, native BTC/SOL) and its prices are stored in asset_price. FALSE with token_id NULL is a configuration defect the fetcher refuses.';
...
-- prices land in asset_price. IDs verified against the CoinGecko API.
...
INSERT INTO offchain_price_asset (source_id, source_asset_id, token_id, name, symbol, enabled, tokenless)
...
VALUES ('20260901_140000_create_asset_price.sql')
