...
COMMENT ON COLUMN position_current.quantity IS 'Derived (copy of position_state.quantity). Holder balance in the instrument''s native units; a zero is a real observation of an emptied position, not a gap. Scale is source-defined and NOT normalized across protocols, exactly as on position_state.quantity, so a consumer must not SUM across heterogeneous instruments without normalizing first. A row carries NO liveness guarantee: a position that simply stops being observed -- a retired projection, or an event-driven one that misses a drain through a path it does not decode -- keeps its last nonzero quantity here indefinitely, with no error and no divergence to detect, because the cache and the spine''s newest row agree and the staleness is in the spine itself. A cache-versus-spine reconciliation cannot catch this class; the only consumer-side signal is the age of as_of_date per projection, so an exposure-style sum should weigh observation age rather than treat every row as equally current.';
