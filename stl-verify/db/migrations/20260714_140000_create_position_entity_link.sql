...
--                non-unique: no FK). The current entity (and its surrogate, if needed) is resolved
--                live by joining entity_master_current on entity_id; we deliberately do NOT freeze a
--                per-version entity_sk here, because a frozen surrogate pins to one SCD2 version and
--                goes stale on the next (Simon review on #574). Point-in-time reads use valid_from.
...
COMMENT ON COLUMN position_entity_link.entity_role IS
  'PK. FK->counterparty_role_ref.counterparty_role. The role this entity plays on the position.';
