...
--
-- The issuer entity_sk is stamped directly onto the position (VEC-421); this table holds the other
-- roles, so a position can carry several linked entities without widening the position row. Written
-- by VEC-421 using the holder/counterparty resolver (VEC-417 -> entity_ref_codes / entity_master).
--
-- Keys and refs:
...
--   entity_id    soft ref to entity_master.entity_id (SCD2 natural key, non-unique: no FK; resolve
--                via entity_master_current).
--   entity_sk    the entity_master surrogate frozen at link time. Snapshot value (not an FK), so the
--                link keeps pointing at the version it was resolved against even as entity_master gains
--                later versions, exactly like the security_sk / issuer entity_sk stamped onto positions.
-- Operational (mutable) like position_classification: re-pointing a role updates its row, so the
-- table is not append-only. valid_from records when the current linkage took effect.
...
CREATE TABLE IF NOT EXISTS position_entity_link (
    ...
    entity_sk     bigint      NOT NULL,   -- entity_master surrogate frozen at link time (snapshot; not an FK)
...
COMMENT ON TABLE position_entity_link IS
  '[Operational] Non-issuer entities linked to a position (VEC-415). One row per (position_id, entity_role): counterparty / custodian / operator / etc. The issuer entity_sk is stamped on the position itself; these are the other roles. Populated by VEC-421 via the holder/counterparty resolver (VEC-417).';
COMMENT ON COLUMN position_entity_link.position_id IS
  'The bytea(32) position identity from position_id() (VEC-400). Part of PK. No FK to a positions table (positions are materialized per protocol).';
COMMENT ON COLUMN position_entity_link.entity_role IS
  'FK->counterparty_role_ref.counterparty_role. The role this entity plays on the position. Part of PK.';
COMMENT ON COLUMN position_entity_link.entity_id IS
  'Soft ref to entity_master.entity_id (resolve via entity_master_current; not an FK).';
COMMENT ON COLUMN position_entity_link.entity_sk IS
  'entity_master surrogate frozen at link time. Snapshot value, not an FK, so it keeps pointing at the resolved version.';
COMMENT ON COLUMN position_entity_link.valid_from IS
  'Date the current linkage took effect (provenance).';
...
-- Reverse lookups: all positions linked to a given entity (by natural key and by frozen surrogate).
CREATE INDEX IF NOT EXISTS position_entity_link_entity_id_idx ON position_entity_link (entity_id);
CREATE INDEX IF NOT EXISTS position_entity_link_entity_sk_idx ON position_entity_link (entity_sk);
