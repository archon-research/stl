...
-- morpho_market_position on timestamp, its partition column -- a bound outside the view does not pass the
-- DISTINCT ON. One difference from the view: the direction carry cannot see a non-zero observation before
-- the bound, so a net-zero row with no direction inside the window takes the direction of the position's
-- latest STORED observation. Bootstrap is unbounded, so that history exists whenever a window is used.
CREATE OR REPLACE FUNCTION position_morpho_market_since(p_since timestamptz)
    RETURNS SETOF position_morpho_market
...
legs AS (
    SELECT s.user_id, s.morpho_market_id,
           encode(m.market_id, 'hex') || ':' || encode(lt.address, 'hex') AS instrument_key,
           s.net_loan AS quantity,
           CASE coalesce(s.is_loan, h.is_loan) WHEN true THEN 'LOAN' WHEN false THEN 'BORROW' END AS deal_type,
           s.block_number, s.block_version, s.processing_version, s.block_timestamp
    FROM series s
    JOIN morpho_market m ON m.id = s.morpho_market_id
    JOIN token lt ON lt.id = m.loan_token_id
    JOIN "user" u ON u.id = s.user_id
    -- The stored direction, only for a row the window left directionless: a stored zero-row already
    -- carries the direction it closed, so the latest stored row is the latest non-zero direction.
    LEFT JOIN LATERAL (
        SELECT ps.deal_type = 'LOAN' AS is_loan
        FROM position_state ps
        WHERE s.is_loan IS NULL
          AND ps.position_id = public.position_id(m.chain_id, m.protocol_id,
                                                  encode(m.market_id, 'hex') || ':' || encode(lt.address, 'hex'),
                                                  encode(u.address, 'hex'))
          AND ps.deal_type IN ('LOAN', 'BORROW')
          AND (ps.block_number, ps.block_version, ps.processing_version)
              < (s.block_number, s.block_version, s.processing_version)
        ORDER BY ps.block_number DESC, ps.block_version DESC, ps.processing_version DESC
        LIMIT 1) h ON true
    UNION ALL
    SELECT s.user_id, s.morpho_market_id,
           encode(m.market_id, 'hex') || ':' || encode(ct.address, 'hex'),
...
SELECT m.chain_id, m.protocol_id, l.instrument_key, encode(u.address, 'hex'), l.quantity, l.deal_type,
       l.block_number, l.block_version, l.processing_version, l.block_timestamp
FROM legs l
JOIN morpho_market m ON m.id = l.morpho_market_id
JOIN "user"        u ON u.id = l.user_id;
...
COMMENT ON FUNCTION position_morpho_market_since(timestamptz) IS '[Operational] VEC-566 bounded source of position_morpho_market: the same rows as the view for observations with morpho_market_position.timestamp after p_since, the bound inside the source scan so a windowed materialize_position_projection() run opens only the chunks after it. A net-zero loan-leg row whose last non-zero observation predates the bound takes its direction from the position''s latest stored observation, so the result equals the view whenever the stored history covers what the window cannot see. Must otherwise stay the view''s SELECT plus that one predicate.';
