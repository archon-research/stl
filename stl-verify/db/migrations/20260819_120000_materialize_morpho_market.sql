...
BEGIN
    ...
    -- Five per class (the negative class is capped in the loop above), or one class's offenders hide another's.
    -- These read the (user, market) pairs from morpho_market_position_current rather than the history.
    SELECT nullif(concat_ws('; ', nullif(array_to_string(v_neg, '; '), ''), string_agg(msg, '; ' ORDER BY cls, msg)), '')
      INTO v_bad
    FROM (
...
COMMENT ON FUNCTION materialize_morpho_market_refusals(timestamptz) IS '[Operational] VEC-402: why materialize_morpho_market() would refuse to run, NULL when it may run, naming up to five offenders per class: a negative source supply, borrow or collateral amount with no higher processing_version for the same user, market, block, block_version and timestamp, which the loan leg''s abs() would launder into a plausible exposure, judged only for observations after p_since when given; one address held by several "user" rows, which renders one position_id since holder_id carries the address alone; a holder address that is not 20 bytes; and a market whose loan or collateral token is on another chain, which the address-keyed legs would merge. The identity checks read the (user, market) pairs from morpho_market_position_current, so they assume that cache is complete: a history load that bypassed its trigger must re-run its backfill (20260909_150100) first.';
...
BEGIN
    IF p_window IS NOT NULL THEN
        ...
        END IF;
    END IF;
    v_bad := materialize_morpho_market_refusals(v_since);
...
COMMENT ON FUNCTION materialize_morpho_market(integer, bigint, interval) IS '[Operational] VEC-402: appends Morpho market position observations into position_state via materialize_position_projection(position_morpho_market). Rejects an invalid p_window first, then refuses to run on any message from materialize_morpho_market_refusals(), judged over the same window. See that function''s comment for the run contract. p_build_id and p_run_id are stamped on every row appended (ADR-0006 §2). p_window is forwarded to the materializer, which bounds the batch it reads; against this view it filters rows without pruning chunks.';
