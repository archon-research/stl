...
--
-- Stamped right after the tables it alters, ahead of every consumer: the derived caches copy the
-- column, and their own writers are parsed when they are created, not when they first run.
...
INSERT INTO migrations (filename) VALUES ('20260818_150000_add_run_id_to_position_stack.sql') ON CONFLICT (filename) DO NOTHING;
