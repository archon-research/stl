...
COMMENT ON COLUMN morpho_adapter.removed_at_block IS 'Block at which the adapter was de-registered; NULL while the adapter is active. Not a plain LEAST: the writer selects the incarnation live at the observed block and takes the minimum of that row''s recorded close and the new observation, and only when the two are within 64 blocks (Ethereum''s reorg bound) of each other in EITHER direction. Within that symmetric window a reorg that re-lands the RemoveAdapter in a neighbouring block therefore settles on the same value in either arrival order. Outside it — above OR below the recorded close — the removal is rejected rather than converged away: the row has conflated two of the adapter''s lifetimes, and either convergence would erase one of two real de-registrations. Full rules: outbound.MorphoRepository.MarkAdapterRemoved.';
