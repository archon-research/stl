...
-- allocation_position on created_at, its partition column -- a bound outside the view does not pass the
-- DISTINCT ON. Single STABLE SELECT, so it inlines and a literal argument is constified at plan time.
CREATE OR REPLACE FUNCTION position_prime_allocation_since(p_since timestamptz)
    RETURNS SETOF position_prime_allocation
    LANGUAGE sql STABLE AS $fn$
SELECT o.chain_id,
       NULL::bigint,
       encode(o.proxy_address, 'hex') || ':' || encode(t.address, 'hex'),
...
COMMENT ON FUNCTION position_prime_allocation_since(timestamptz) IS '[Operational] VEC-566 bounded source of position_prime_allocation: the same rows as the view for observations with allocation_position.created_at after p_since, the bound inside the source scan so a windowed materialize_position_projection() run opens only the chunks after it. Must stay the view''s SELECT plus that one predicate.';
