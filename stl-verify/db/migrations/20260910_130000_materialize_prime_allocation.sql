...
CREATE OR REPLACE FUNCTION materialize_prime_allocation(p_build_id integer DEFAULT 0,
    ...
    LANGUAGE plpgsql
    -- Empty, so every reference in the body below is schema-qualified.
    SET search_path = ''
...
BEGIN
    SELECT string_agg(msg, '; ' ORDER BY msg) INTO v_bad FROM (
        SELECT msg FROM (
            ...
            -- two positions carrying one proxy's balance -- a double count the spine cannot see. Keyed
            -- per block: a proxy reassigned between primes is legitimate, two at one block is not.
            SELECT format('chain %s proxy %s token %s is held by %s primes (%s) at block %s/%s', ap.chain_id,
                          encode(ap.proxy_address, 'hex'), encode(t.address, 'hex'),
                          count(DISTINCT ap.prime_id),
                          string_agg(DISTINCT encode(pr.vault_address, 'hex'), ','),
                          ap.block_number, ap.block_version) AS msg
            FROM public.allocation_position ap
            JOIN public.token t  ON t.id = ap.token_id
            JOIN public.prime pr ON pr.id = ap.prime_id
            GROUP BY ap.chain_id, ap.proxy_address, t.address, ap.block_number, ap.block_version
            HAVING count(DISTINCT ap.prime_id) > 1
            UNION ALL
            -- None of the three carries a length CHECK. A short vault fails the spine's 40-hex holder
            -- check; a short token or proxy renders a non-blank instrument_key that position_key()
            -- accepts, so it mints a plausible wrong identity with no error at all.
            SELECT format('chain %s: proxy %s is %s bytes, holder %s is %s bytes, token %s is %s bytes',
                          ap.chain_id,
                          encode(ap.proxy_address, 'hex'), length(ap.proxy_address),
                          encode(pr.vault_address, 'hex'), length(pr.vault_address),
                          encode(t.address, 'hex'), length(t.address))
            FROM public.allocation_position ap
            JOIN public.token t  ON t.id = ap.token_id
            JOIN public.prime pr ON pr.id = ap.prime_id
            WHERE length(pr.vault_address) <> 20 OR length(t.address) <> 20 OR length(ap.proxy_address) <> 20
            GROUP BY ap.chain_id, ap.proxy_address, pr.vault_address, t.address
            UNION ALL
            -- chain_id is in position_key()'s pre-image, so a row whose chain disagrees with its token's
            -- does not collide: it keys a different, silently wrong position nothing can trace back.
    ...
    END IF;
    RETURN public.materialize_position_projection('public.position_prime_allocation'::regclass, p_build_id, p_run_id, p_window);
END
$fn$;

COMMENT ON FUNCTION materialize_prime_allocation(integer, bigint, interval) IS '[Operational] VEC-407: appends Prime ALM allocation observations into position_state via materialize_position_projection(position_prime_allocation). Refuses to run, naming up to five offenders, when one (chain, proxy, token) is held by several primes, since the prime is carried by holder_id alone and both rows would store the same proxy balance, when a row''s chain_id disagrees with its token''s, since both render one instrument_key, or when a vault, token or proxy address cannot render a usable identity. COST: the three checks are whole-table aggregations of allocation_position, a tiered columnstore hypertable, and enable_tiered_reads is pinned on, so they read all of history on every call. p_window bounds the batch the spine reads but not these, so a windowed run still pays them in full; the multi-prime check is keyed per block so it cannot be bounded by wall-clock time without missing an old block. See that function''s comment for the run contract. p_build_id and p_run_id are stamped on every row appended (ADR-0006 §2). p_window is forwarded to the materializer, which bounds the batch it reads; against this view it filters rows without pruning chunks.';
