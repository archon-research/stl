...
-- Ordered after the spine's own migration: LANGUAGE sql parses the body at creation, so the
-- materializer must already exist. search_path is empty and both references are schema-qualified.
