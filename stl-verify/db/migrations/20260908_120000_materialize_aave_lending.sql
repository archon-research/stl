...
-- Bounded source for a windowed run (VEC-566): the view's SELECT with the bound INSIDE both ledger
-- scans on created_at, their partition column -- a bound outside the view does not pass the DISTINCT ON.
-- Single STABLE SELECT, so it inlines and a literal argument is constified at plan time.
CREATE OR REPLACE FUNCTION position_aave_lending_since(p_since timestamptz)
    RETURNS SETOF position_aave_lending
...
FROM (
    ...
    (SELECT DISTINCT ON (c.user_id, c.protocol_id, c.token_id, c.block_number, c.block_version, c.processing_version)
           ...
           encode(rt.receipt_token_address, 'hex') AS instrument_key
    FROM borrower_collateral c
    LEFT JOIN (SELECT protocol_id, underlying_token_id,
                      CASE WHEN count(*) = 1 THEN min(receipt_token_address) END AS receipt_token_address
               FROM receipt_token GROUP BY protocol_id, underlying_token_id) AS rt
           ON rt.protocol_id = c.protocol_id AND rt.underlying_token_id = c.token_id
...
COMMENT ON FUNCTION position_aave_lending_since(timestamptz) IS '[Operational] VEC-566 bounded source of position_aave_lending: the same rows as the view for ledger rows with created_at after p_since on both borrower and borrower_collateral, the bound inside each source scan so a windowed materialize_position_projection() run opens only the chunks after it. The caller resolves the instant (a plain high-water mark skips rows a later build writes into older chunks). Must stay the view''s SELECT plus that one predicate per leg.';
