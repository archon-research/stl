...
COMMENT ON TABLE uniswap_v4_position_nft_transfer IS
  '[Timeseries] One row per ERC-721 Transfer log emitted by the Uniswap V4 PositionManager: a posm position NFT being minted, moved or burned. The holder of token_id at block N is the newest row at or below it: join uniswap_v4_position_manager on chain_id and take every version of it (never one id -- a registry correction starts a new surrogate and the rows written before it keep the old one), exclude the (block_number, block_version) pairs block_states marks is_orphaned, then ORDER BY block_number DESC, block_version DESC, log_index DESC, processing_version DESC LIMIT 1; its to_address is the answer, all zeros meaning a burn rather than a holder. log_index is part of that order because a token can change hands twice in one block. The runbook section "Who holds a posm position NFT" carries the query verbatim, with the two traps that reading it back by hand runs into. Every field is carried by the log itself, so a reorg redelivery re-decodes the new fork''s logs and appends them at the new block_version; nothing is ever re-read from chain state, which also means a row decoded on a fork the watcher later orphaned is superseded only if the canonical block re-emits a transfer of that token -- which is why the holder query excludes the pairs block_states marks is_orphaned, and why it can only answer for the 30 days block_states is retained. Created plain, not as a hypertable, under the create-plain rule in db/migrations/AGENTS.md: mainnet emits about a thousand posm transfers a day (290 logs over 2,001 blocks on 2026-08-31), and partitioning is a later migration made once measurement calls for it. The VectorUniswapV4NFTTransferGrowthHigh row-growth alert is that measurement (250k rows/day sustained), and its runbook section carries the conversion -- block_timestamp joins the PK, create_hypertable with migrate_data, 30-day chunks, compression segmented by position_manager_id behind the VEC-615 version function, and no tiering policy unless the holder read is bounded inside the horizon, because that read has no lower bound and timescaledb.enable_tiered_reads is off. Append-only via the processing_version trigger.';
