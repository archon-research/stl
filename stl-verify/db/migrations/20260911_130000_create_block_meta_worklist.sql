...
COMMENT ON TABLE block_meta_worklist IS '[Operational] VEC-491 run scratch: the blocks a chain references that block_meta lacks, enumerated once per run and paged with a keyset cursor. NOT history and never read as of a block: rows are fully reproducible by re-running the enumeration, which is why it is UNLOGGED. A run that reaches the end of its list clears its own chain; rows that survive therefore mean an interrupted run, and the next one resumes them instead of re-enumerating.';
