...
COMMENT ON COLUMN block_meta_worklist.run_id IS 'Roles: PK, FK→writer_run.id. An ownership key, not the audit-only run_id ADR-0006 puts on governed tables: it is NOT NULL, it is part of the primary key, and it names the run that enumerated this row and the only writer that deletes it. Rows of a run that died are swept by the next run on any chain once the owning run started more than 48 hours ago, twice the loader''s 24-hour activity ceiling.';
