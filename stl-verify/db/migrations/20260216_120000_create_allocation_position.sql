-- Create allocation_position table for tracking spark/grove ALM proxy token positions.
-- Each row is a historical snapshot: the balance after a Transfer event that touched the proxy.
...
CREATE TABLE IF NOT EXISTS allocation_position (
                                                   ...
                                                   UNIQUE (chain_id, block_number, block_version, tx_hash, log_index, direction)
);

-- Current positions: latest snapshot per position
CREATE INDEX idx_allocation_position_current ON allocation_position (chain_id, token_id, star, proxy_address, block_number DESC);

-- History by star
CREATE INDEX idx_allocation_position_star ON allocation_position (star, chain_id, block_number DESC);

-- History by token
CREATE INDEX idx_allocation_position_token ON allocation_position (token_id, chain_id, block_number DESC);
