...
-- reserve columns say how big the pool is; these three say how much of it
...
ALTER TABLE psm3_reserves
    ADD COLUMN spark_alm_shares      NUMERIC,   -- raw 1e18, PSM3.shares(sparkAlm)
...
COMMENT ON COLUMN psm3_reserves.spark_alm_shares IS
  'PSM3.shares(sparkAlm) — the internal LP shares held by Spark''s ALM proxy, today the only meaningful depositor (a future prime''s stake would get its own {prime}_alm columns). Raw on-chain integer at 1e18: shares are minted in the contract''s own 18-decimal USD value unit (the 1 USDC genesis deposit mints 1e18 shares), and the value per share drifts up from there, so this is NOT a token amount and must not be scaled by any token.decimals. Shares are internal accounting only — no share token, no transfers, no ERC-4626 surface — so the holder cannot be discovered from chain state; the ALM proxy address is pinned per chain in the indexer config and cross-checked against axis-synome at startup, and is not stored in this table. Use spark_alm_shares / total_shares for the ownership fraction and spark_alm_asset_value for the USD figure. NULL on rows written before the share legs were indexed.';
...
COMMENT ON COLUMN psm3_reserves.spark_alm_asset_value IS
  'PSM3.convertToAssetValue(spark_alm_shares) — the ALM proxy''s stake valued at par, read on-chain rather than derived. Raw on-chain integer, 18-decimal USD, the same par scale as total_assets (not a market-priced value). Equals spark_alm_shares x total_assets / total_shares with the contract''s truncating division, so recomputing it off-chain can differ in the last units; prefer this column. NULL on rows written before the share legs were indexed.';
