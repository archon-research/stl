...
func TestLiveService_TryAddBlock_SkipsDuplicates(t *testing.T) {
	service := &LiveService{
		config: LiveConfig{
			MaxUnfinalizedBlocks: 100,
		},
	...
	added1 := service.tryAddBlock(block)
	added2 := service.tryAddBlock(block)

	if !added1 {
		t.Error("first add should succeed")
	}
	if added2 {
		t.Error("second add should be rejected as duplicate")
	}
