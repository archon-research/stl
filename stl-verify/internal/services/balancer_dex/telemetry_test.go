...
// balancer.blocks.processed datapoint. Without this the Balancer stall
// alert in alerts/vector-indexers.yaml cannot fire and prod has no pager on
// a stalled worker.
func TestProcessBlockEvent_EmitsBlockProcessedMetric(t *testing.T) {
	...
	// Happy path: empty receipts list.
	body := []byte(`[]`)
	...
	success, errCount := readBlockCountersByStatus(t, &rm, "balancer.blocks.processed")
	if success != 1 {
		t.Errorf("balancer.blocks.processed{status=success} = %d, want 1", success)
	}
	if errCount != 1 {
		t.Errorf("balancer.blocks.processed{status=error} = %d, want 1", errCount)
	}
}
