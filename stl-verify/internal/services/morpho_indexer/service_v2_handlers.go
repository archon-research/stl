...
func (s *Service) handleAddAdapter(ctx context.Context, e *AddAdapterEvent, vaultAddress common.Address, blockNumber int64) error {
	...
	return s.txManager.WithTransaction(ctx, func(tx pgx.Tx) error {
		_, err := s.upsertAdapterRow(ctx, tx, vault, vaultAddress, e.Account, adapterType, blockNumber, adapterPathAddAdapter)
		return err
	})
}
...
//
// path names the caller for telemetry only; it is the single place all three
// registration routes converge, so it is also where the registration counter the
// unknown-adapter and lazy-registration alerts read is recorded.
