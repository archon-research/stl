...
// the same answers. Each chunk is fetched as several per-address-batch requests
// whose results interleave, so the node's per-response ordering is not enough.
