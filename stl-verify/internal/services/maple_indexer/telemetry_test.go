...
// TestSecondsHistograms_UseSecondsBuckets guards against the bucket-boundary
// bug behind the RPC-latency alerts (see the equivalent morpho_indexer test).
// Without explicit boundaries the SDK applies its default millisecond-scale
// buckets ([0,5,10,...]), so a seconds-valued metric collapses into the (0,5]
// bucket and histogram_quantile(0.99,...) interpolates to 0.99*5 = 4.95s,
// tripping the >3s alert permanently. Every seconds histogram must instead use
...
func TestSecondsHistograms_UseSecondsBuckets(t *testing.T) {
	...
	tel.RecordRPCCall(ctx, "getVaultState", 30*time.Millisecond, nil)
...
func exerciseAllMethods(t *testing.T, tel *Telemetry) {
	...
	tel.RecordVaultStateWrite(ctx)
	tel.RecordPositionWrites(ctx, 3)
	tel.RecordRPCCall(ctx, "getVaultState", time.Millisecond, nil)
	tel.RecordRPCCall(ctx, "getVaultState", time.Millisecond, someErr)
...
func TestTelemetry_NilSafe(t *testing.T) {
	...
	t.Run("RecordVaultStateWrite", func(t *testing.T) {
		tel.RecordVaultStateWrite(ctx)
	})

	t.Run("RecordPositionWrites", func(t *testing.T) {
		tel.RecordPositionWrites(ctx, 3)
	})

	t.Run("RecordRPCCall", func(t *testing.T) {
		tel.RecordRPCCall(ctx, "getVaultState", 100*time.Millisecond, nil)
		tel.RecordRPCCall(ctx, "getVaultState", 100*time.Millisecond, someErr)
	})
