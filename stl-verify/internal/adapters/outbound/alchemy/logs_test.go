...
func TestGetLogs_SendsHexBoundedFilterWithTopicOrSet(t *testing.T) {
	var captured []any
	...
	_, err := client.GetLogs(context.Background(), outbound.LogFilter{
		...
		Topic1: []common.Hash{
			common.HexToHash("0x1d5b2949ece8754c2d736991c62c5162bd144f497b2212182401b9bae77e2d76"),
			common.HexToHash("0xbc21dd4a44766fadfd6447f4b222a6185dcc2e6a3b15eb79e0cc637e30e7e97f"),
		},
	})
	if err != nil {
	...
	if got.FromBlock != "0x14af009" {
		t.Errorf("fromBlock = %q, want 0x14af009", got.FromBlock)
	}
	if got.ToBlock != "0x14b1da0" {
		t.Errorf("toBlock = %q, want 0x14b1da0", got.ToBlock)
	}
	if !strings.EqualFold(got.Address, "0x000000000004444c5dc75cB358380D2e3dE08A90") {
		t.Errorf("address = %q", got.Address)
	}
	if len(got.Topics) != 2 {
		t.Fatalf("topics length = %d, want 2 (topic0 then the topic1 OR-set)", len(got.Topics))
	}
	if topic0, ok := got.Topics[0].(string); !ok || !strings.EqualFold(topic0, "0xf208f4912782fd25c7f114ca3723a2d5dd6f3bcc3ac8db5af63baa85f711d5ec") {
		t.Errorf("topics[0] = %v, want the ModifyLiquidity topic0 as a scalar", got.Topics[0])
	}
	orSet, ok := got.Topics[1].([]any)
	if !ok {
		t.Fatalf("topics[1] = %T, want a JSON array (the pool-id OR-set)", got.Topics[1])
	}
	if len(orSet) != 2 {
		t.Errorf("topics[1] length = %d, want 2", len(orSet))
	}
}

func TestGetLogs_OmitsTopic1WhenUnconstrained(t *testing.T) {
	var captured []any
	client := logsTestClient(t, func(w http.ResponseWriter, r *http.Request) {
		var req jsonRPCRequest
		_ = json.NewDecoder(r.Body).Decode(&req)
	...
	if len(got.Topics) != 1 {
		t.Fatalf("topics length = %d, want 1 (topic0 only)", len(got.Topics))
	}
}
...
func TestClientImplementsLogScanClient(t *testing.T) {
