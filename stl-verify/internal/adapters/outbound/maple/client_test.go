...
func TestNewClient(t *testing.T) {
	tests := []struct {
		...
		},
		{
			...
			config: Config{
				Endpoint: "https://custom.maple.finance/v2/graphql",
...
func TestNewClient_defaults(t *testing.T) {
	...
	if client.endpoint != "https://api.maple.finance/v2/graphql" {
		t.Errorf("endpoint = %q, want default", client.endpoint)
	}
}
...
func TestGetAccountPositions(t *testing.T) {
	tests := []struct {
		...
		wantErr        bool
		errContains    string
		checkAddress   bool // verify address was lowercased in request
		checkPositions bool
	}{
		{
			name:    "success: single position",
			address: common.HexToAddress("0x1601843C5E9BC251A3272907010AFA41FA18347E"), // mixed case
						...
						"poolV2Positions": [
							{
								...
								"lendingBalance": "200000000000000"
							}
						]
					}
				}`),
		...
		{
			name:    "success: multiple positions",
				...
				Data: json.RawMessage(`{
					"account": {
						"poolV2Positions": [
							...
							{
								"pool": {
									"id": "0x356b8d89c1e1239cbbb9de4815c39a1474d5ba7d",
									"name": "Syrup USDT",
									"asset": {"symbol": "USDT", "decimals": 6}
								},
								"lendingBalance": "55000000000000"
							}
						]
					}
				}`),
		...
		},
		{
			...
			serverResponse: graphqlResponse{
				Data: json.RawMessage(`{"account": null}`),
			},
			serverStatus: http.StatusOK,
			wantCount:    0,
		},
		...
		{
			name:    "invalid lending balance: non-numeric",
			address: common.HexToAddress("0x1601843c5e9bc251a3272907010afa41fa18347e"),
			serverResponse: graphqlResponse{
				Data: json.RawMessage(`{
					"account": {
			...
			serverStatus: http.StatusOK,
			wantErr:      true,
			errContains:  "parsing lending balance",
		},
		{
			name:           "HTTP error: 500",
			address:        common.HexToAddress("0x1601843c5e9bc251a3272907010afa41fa18347e"),
	...
	for _, tc := range tests {
		t.Run(tc.name, func(t *testing.T) {
			...
			server := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
				...
				// Parse request body to capture variables.
				var req graphqlRequest
				if err := json.NewDecoder(r.Body).Decode(&req); err == nil {
					if addr, ok := req.Variables["addr"].(string); ok {
			...
			positions, err := client.GetAccountPositions(ctx, tc.address)
			if tc.wantErr {
				if err == nil {
			...
			if len(positions) != tc.wantCount {
				t.Fatalf("count = %d, want %d", len(positions), tc.wantCount)
			}

			if tc.checkAddress {
				wantAddr := strings.ToLower(tc.address.Hex())
				if capturedAddress != wantAddr {
					t.Errorf("captured address = %q, want %q (lowercased)", capturedAddress, wantAddr)
				}
			}

			if tc.checkPositions && len(positions) > 0 {
				pos := positions[0]
				wantPoolAddr := common.HexToAddress("0x80ac24aa929eaf5013f6436cda2a7ba190f5cc0b")
				if pos.PoolAddress != wantPoolAddr {
					t.Errorf("PoolAddress = %s, want %s", pos.PoolAddress.Hex(), wantPoolAddr.Hex())
				}
				if pos.PoolName != "Syrup USDC" {
					t.Errorf("PoolName = %q", pos.PoolName)
				}
				if pos.AssetSymbol != "USDC" {
					t.Errorf("AssetSymbol = %q", pos.AssetSymbol)
				}
				if pos.AssetDecimals != 6 {
					t.Errorf("AssetDecimals = %d", pos.AssetDecimals)
				}
				expected := big.NewInt(200_000_000_000_000)
				if pos.LendingBalance.Cmp(expected) != 0 {
					t.Errorf("LendingBalance = %s, want %s", pos.LendingBalance, expected)
				}
			}
		})
	}
...
func TestExecute_contextCanceled(t *testing.T) {
	...
	_, err = client.GetAccountPositions(ctx, common.HexToAddress("0x1601843c5e9bc251a3272907010afa41fa18347e"))
	if err == nil {
