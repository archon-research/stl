...
func NewReceiptTokenRepository(pool *pgxpool.Pool, logger *slog.Logger, runID buildregistry.RunID) (*ReceiptTokenRepository, error) {
	if pool == nil {
	...
	return &ReceiptTokenRepository{pool: pool, logger: logger, runID: runID}, nil
}
...
func (r *ReceiptTokenRepository) GetOrCreateReceiptToken(ctx context.Context, tx pgx.Tx, token entity.ReceiptToken) (int64, error) {
	var id int64
	err := tx.QueryRow(ctx,
		`INSERT INTO receipt_token (chain_id, protocol_id, underlying_token_id, receipt_token_address, symbol, created_at_block, metadata, updated_at, run_id)
		 ...
		 ON CONFLICT (chain_id, receipt_token_address) DO UPDATE SET
		     ...
		     updated_at = NOW()
		 RETURNING id`,
		token.ChainID, token.ProtocolID, token.UnderlyingTokenID, token.ReceiptTokenAddress.Bytes(), token.Symbol, token.CreatedAtBlock, r.runID,
	).Scan(&id)
	if err != nil {
