...
// TestLargeDataset_Setup is a test that sets up the large dataset.
// Run this first to populate the database, then run benchmarks.
...
// BenchmarkQueries provides standard Go benchmarks for the queries.
// These run with the default b.N iterations for accurate measurements.
func BenchmarkQueries(b *testing.B) {
	if testing.Short() {
		b.Skip("Skipping benchmark in short mode")
	}

	repo, cleanup := setupLargePostgres(b)
	defer cleanup()

	ctx := context.Background()

	// Insert a smaller dataset for benchmarks (1 million is enough)
	const benchmarkRows = 1_000_000
	bulkInsertBlocksCOPY(b, repo.DB(), 1, benchmarkRows)
	createIndexes(b, repo.DB())

	b.ResetTimer()

	b.Run("GetLastBlock", func(b *testing.B) {
		for i := 0; i < b.N; i++ {
			repo.GetLastBlock(ctx)
		}
	})

	b.Run("GetBlockByNumber", func(b *testing.B) {
		for i := 0; i < b.N; i++ {
			repo.GetBlockByNumber(ctx, int64(i%benchmarkRows)+1)
		}
	})

	b.Run("GetBlockByHash", func(b *testing.B) {
		for i := 0; i < b.N; i++ {
			hash := fmt.Sprintf("0x%064d", int64(i%benchmarkRows)+1)
			repo.GetBlockByHash(ctx, hash)
		}
	})

	b.Run("GetMinBlockNumber", func(b *testing.B) {
		for i := 0; i < b.N; i++ {
			repo.GetMinBlockNumber(ctx)
		}
	})

	b.Run("GetMaxBlockNumber", func(b *testing.B) {
		for i := 0; i < b.N; i++ {
			repo.GetMaxBlockNumber(ctx)
		}
	})

	b.Run("GetRecentBlocks_10", func(b *testing.B) {
		for i := 0; i < b.N; i++ {
			repo.GetRecentBlocks(ctx, 10)
		}
	})

	b.Run("GetRecentBlocks_100", func(b *testing.B) {
		for i := 0; i < b.N; i++ {
			repo.GetRecentBlocks(ctx, 100)
		}
	})

	b.Run("GetBlockVersionCount", func(b *testing.B) {
		for i := 0; i < b.N; i++ {
			repo.GetBlockVersionCount(ctx, int64(i%benchmarkRows)+1)
		}
	})

	b.Run("GetBackfillWatermark", func(b *testing.B) {
		for i := 0; i < b.N; i++ {
			repo.GetBackfillWatermark(ctx)
		}
	})

	b.Run("FindGaps_1000", func(b *testing.B) {
		repo.SetBackfillWatermark(ctx, benchmarkRows-1000)
		b.ResetTimer()
		for i := 0; i < b.N; i++ {
			repo.FindGaps(ctx, benchmarkRows-1000, benchmarkRows)
		}
	})

	b.Run("VerifyChainIntegrity_1000", func(b *testing.B) {
		for i := 0; i < b.N; i++ {
			start := int64(i%int(benchmarkRows-1000)) + 1
			repo.VerifyChainIntegrity(ctx, start, start+1000)
		}
	})

	b.Run("GetBlocksWithIncompletePublish_10", func(b *testing.B) {
		for i := 0; i < b.N; i++ {
			repo.GetBlocksWithIncompletePublish(ctx, 10, false)
		}
	})

	b.Run("SaveBlock", func(b *testing.B) {
		nextBlock := int64(benchmarkRows + 1)
		b.ResetTimer()
		for i := 0; i < b.N; i++ {
			repo.SaveBlock(ctx, outbound.BlockState{
				Number:     nextBlock,
				Hash:       fmt.Sprintf("0xbench_%d_%d", nextBlock, i),
				ParentHash: fmt.Sprintf("0x%064d", nextBlock-1),
				ReceivedAt: time.Now().Unix(),
			})
			nextBlock++
