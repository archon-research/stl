...
func (r *PrimeDebtRepository) SaveDebtSnapshots(ctx context.Context, debts []*entity.PrimeDebt) error {
	...
	// Sort by natural key so the per-row advisory lock in
	// assign_processing_version_prime_debt is acquired in a transaction-stable
	// order across concurrent callers. See ADR-0002 §3.
