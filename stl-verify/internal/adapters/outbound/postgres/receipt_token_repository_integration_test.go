...
func TestGetOrCreateReceiptToken(t *testing.T) {
	tests := []struct {
		name string
		...
		{
			name: "idempotent returns same ID",
			run: func(t *testing.T, ctx context.Context) {
				truncateReceiptToken(t, ctx)
				protocolID, tokenID := seedReceiptTokenDeps(t, ctx, 1)
		...
		{
			name: "created_at_block uses LEAST on conflict",
			run: func(t *testing.T, ctx context.Context) {
				truncateReceiptToken(t, ctx)
				protocolID, tokenID := seedReceiptTokenDeps(t, ctx, 1)
		...
		{
			name: "different chain same address creates new row",
			run: func(t *testing.T, ctx context.Context) {
				truncateReceiptToken(t, ctx)
