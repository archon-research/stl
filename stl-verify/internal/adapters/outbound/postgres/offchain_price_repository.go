...
func NewPriceRepository(pool *pgxpool.Pool, logger *slog.Logger, buildID buildregistry.BuildID, runID buildregistry.RunID, batchSize int) (*PriceRepository, error) {
	if pool == nil {
...
func (r *PriceRepository) GetEnabledAssets(ctx context.Context, sourceID int64) ([]*entity.PriceAsset, error) {
	rows, err := r.pool.Query(ctx, `
		SELECT id, source_id, source_asset_id, token_id, tokenless, name, symbol, enabled, created_at, updated_at
...
func scanPriceAssets(rows pgx.Rows) ([]*entity.PriceAsset, error) {
	...
	for rows.Next() {
		...
		if err := rows.Scan(
			&pa.ID, &pa.SourceID, &pa.SourceAssetID, &pa.TokenID, &pa.Tokenless, &pa.Name, &pa.Symbol, &pa.Enabled, &pa.CreatedAt, &pa.UpdatedAt,
		); err != nil {
...
// UpsertAssetPrices inserts asset-keyed price records (asset_price) in
...
func (r *PriceRepository) UpsertAssetPrices(ctx context.Context, prices []*entity.AssetPrice) error {
	...
	// next_processing_version_asset_price must be acquired in a
...
func (r *PriceRepository) upsertPriceBatch(ctx context.Context, tx pgx.Tx, prices []*entity.TokenPrice) error {
	...
	var sb strings.Builder
	sb.WriteString(`
		INSERT INTO offchain_token_price (token_id, source_id, timestamp, price_usd, market_cap_usd, volume_usd, build_id, run_id)
		VALUES `)
