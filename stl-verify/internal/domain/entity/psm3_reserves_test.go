...
func TestPSM3Reserves_Validate(t *testing.T) {
	tests := []struct {
		...
		{"zero alm position address", func(s *PSM3Reserves) { s.State.ALMPositions[0].Address = common.Address{} }, "alm position address"},
		{"empty alm position prime", func(s *PSM3Reserves) { s.State.ALMPositions[0].Prime = "" }, "alm position prime"},
		{"no alm positions", func(s *PSM3Reserves) { s.State.ALMPositions = nil }, "at least one alm position"},
		{"duplicate alm positions", func(s *PSM3Reserves) {
			s.State.ALMPositions = append(s.State.ALMPositions, s.State.ALMPositions[0])
		}, "duplicate alm position"},
		...
		{"nil alm shares", func(s *PSM3Reserves) { s.State.ALMPositions[0].Shares = nil }, "shares is required"},
		{"nil total shares", func(s *PSM3Reserves) { s.State.TotalShares = nil }, "total_shares"},
		{"nil alm asset value", func(s *PSM3Reserves) { s.State.ALMPositions[0].AssetValue = nil }, "asset_value is required"},
		{"alm shares equal to total shares is fine", func(s *PSM3Reserves) { s.State.ALMPositions[0].Shares = big.NewInt(7) }, ""},
		{"alm shares exceed total shares", func(s *PSM3Reserves) { s.State.ALMPositions[0].Shares = big.NewInt(8) }, "exceeds total_shares"},
		{"alm asset value exceeds total assets", func(s *PSM3Reserves) { s.State.ALMPositions[0].AssetValue = big.NewInt(10) }, "exceeds total_assets"},
