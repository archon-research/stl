...
func validPSM3Reserves() PSM3Reserves {
	return PSM3Reserves{
		...
		State: PSM3State{
			...
			ALMShares:      big.NewInt(6),
			TotalShares:    big.NewInt(7),
			ALMAssetValue:  big.NewInt(8),
		},
		BlockNumber:    100,
		BlockVersion:   0,
...
func TestPSM3Reserves_Validate(t *testing.T) {
	tests := []struct {
		...
		{"nil alm shares", func(s *PSM3Reserves) { s.State.ALMShares = nil }, "alm_shares"},
		{"nil total shares", func(s *PSM3Reserves) { s.State.TotalShares = nil }, "total_shares"},
		{"nil alm asset value", func(s *PSM3Reserves) { s.State.ALMAssetValue = nil }, "alm_asset_value"},
