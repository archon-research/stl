...
func TestNewBorrower(t *testing.T) {
	...
	tests := []struct {
		...
		{
			name:            "nil txHash is valid for MapleSnapshot",
			id:              1,
			userID:          10,
			protocolID:      5,
			protocolAssetID: 3,
			...
			amount:          validAmount,
			change:          validChange,
			eventType:       EventMapleSnapshot,
			txHash:          nil,
			wantErr:         false,
		},
		{
			name:            "empty txHash is valid for MapleSnapshot",
			id:              1,
			userID:          10,
			protocolID:      5,
			protocolAssetID: 3,
		...
		{
			name:            "nil txHash is invalid for non-Maple events",
			id:              1,
			userID:          10,
			protocolID:      5,
			protocolAssetID: 3,
		...
		{
			name:            "empty txHash is invalid for non-Maple events",
			id:              1,
			userID:          10,
			protocolID:      5,
			protocolAssetID: 3,
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			borrower, err := NewBorrower(tt.id, tt.userID, tt.protocolID, tt.protocolAssetID, tt.blockNumber, tt.blockVersion, tt.amount, tt.change, tt.eventType, tt.txHash)
			if tt.wantErr {
				if err == nil {
			...
			if borrower.ProtocolAssetID != tt.protocolAssetID {
				t.Errorf("NewBorrower() ProtocolAssetID = %v, want %v", borrower.ProtocolAssetID, tt.protocolAssetID)
			}
