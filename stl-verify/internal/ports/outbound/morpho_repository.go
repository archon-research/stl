...
// assertion of MEMBERSHIP but was given no classification to record with it.
// Distinguished from any other failure because the caller — not the registry — is what
// has to change: it must supply the on-chain type probe's answer. A removal is exempt:
// an adapter first seen by its own de-registration has no known type and records none.
...
type MorphoRepository interface {
	...
	//
	// Appending an assertion of MEMBERSHIP requires a classification; a nil AdapterType
	// there fails with ErrAdapterUnclassified rather than recording a defaulted type. A
	// removal may be untyped — an adapter first observed by its own removal has no known
	// type, and NULL is the honest record.
	//
	// Idempotent on (adapter, block_number, block_version, log_index): redelivery and replay
	// converge on the same rows in any arrival order. Nothing is ever updated, so a
	// reorg-relocated or re-observed transition is simply another row at its own position
	// and the ordering tuple selects between them — there is no convergence, no relocation
	// bound, and no incarnation for a snapshot to be stranded by.
	...
	GetActiveAdaptersByVault(ctx context.Context, morphoVaultID int64) ([]*entity.MorphoAdapterMember, error)
