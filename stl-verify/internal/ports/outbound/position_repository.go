...
// BorrowerRecord represents a single borrower (debt) record for batch insertion.
type BorrowerRecord struct {
	...
	BlockNumber  int64
	BlockVersion int
	Amount       *big.Int // raw wei full current outstanding debt
	Change       *big.Int // raw wei event delta
	EventType    string
	TxHash       []byte
}
...
type PositionRepository interface {
	...
	SaveBorrowers(ctx context.Context, tx pgx.Tx, records []BorrowerRecord) error
