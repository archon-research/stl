...
type UniswapV4Repository interface {
	...
	// SavePositions persists position rows on their own, through the same
	// append-on-change path SaveBlock's position phase uses, and returns how many
	// rows were actually inserted — zero when every slot's stored state already
	// matches. It exists for the one-shot position bootstrap, whose only write is
	// positions and whose whole point is being able to tell a rerun over covered
	// history (0) from one that closed a real gap.
