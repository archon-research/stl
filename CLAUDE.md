...
- **Function composition** (read code like a book):
    ...
    - Enforced in the Review phase: the code-quality reviewer rejects any new or modified function that violates this. Audit EVERY changed function, not a named subset (scoping the review to specific files creates blind spots, which is how a 254-line function once slipped through). When this rule is newly adopted or changed, retro-audit the already-written in-scope code, not just new code. A function-length / complexity linter (golangci-lint `funlen`/`gocognit`) is the planned deterministic backstop (VEC-465) so an over-long function fails CI automatically rather than relying on a reviewer noticing.
