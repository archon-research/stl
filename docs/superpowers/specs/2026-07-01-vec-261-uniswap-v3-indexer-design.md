...
- **Entrypoint** `cmd/workers/uniswap-v3-indexer/main.go`: `dexbootstrap.ParseConfig` → `Bootstrap` → `LoadPools` → `ResolveProtocolID` (UniswapV3 factory protocol row already seeded in `20260521_100000_create_dex_prereqs.sql`) → `NewProtocolEventWriter` → construct service → `NewBlockProcessor` → `sqsutil.RunLoop`. `run(ctx, args)` for testability; `main()` at top.
