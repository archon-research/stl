...
#### `loanMeta.*` is off-chain editorial metadata — fill-once, NOT immutable
...
**Indexer treatment.** Model `loanMeta.*` as **fill-once**, not immutable: a stored `NULL` may be filled from an incoming value (legitimate late enrichment, must persist), but a stored non-NULL changing (`value → value`) or clearing (`value → null`) is still an error worth failing on. The `maple_loan` upsert implements this via `ON CONFLICT … DO UPDATE SET loan_meta_x = COALESCE(maple_loan.loan_meta_x, EXCLUDED.loan_meta_x)` with a NULL-safe scan guard; `maple_pool_id` and `borrower_user_id` (genuinely on-chain) stay strict. Note `loanMeta.type` filling in late means a loan can be reclassified from external to internal **after** first index — `is_internal` (and any backing aggregation that filters on it) updates only once the type lands.
