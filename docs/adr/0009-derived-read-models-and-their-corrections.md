...
- **It does not claim the refresh scales indefinitely, and the cost is smaller than it first looked.** At the shape the only real source has today — `allocation_position`, 2.2M observations over 88 positions and 11,309 derived keys — a full refresh is 2.2s cold and 1.9s writing nothing, measured on a rig seeded 6x larger than that (65,560 keys). An earlier measurement of 430s came from a fixture with 56,000 positions and 20.4M derived keys, which is roughly 1,800x the real key count and not a shape this system has. The cost is dominated by the per-row comparison against what is already stored rather than by the scan, so it grows with the model's own size: a model that re-offers every key on every tick is bounded by its key count, and the ceiling is worth measuring again before one of these reaches millions of keys. Driving the refresh from what changed instead would need an arrival axis monotonic in commit order, which is `ingest_xid` (ARCT-438), not `created_at` — but nothing today needs it.
