...
- **It does not claim the refresh is cheap.** Measured on a standalone rig at 20.4M source rows across 56,000 positions and 365 dates — 20.4M derived keys — computing the winners takes 7.5s, while the full refresh takes 430s with `ON CONFLICT DO NOTHING` and 300s with an explicit anti-join, both writing nothing. The cost is the per-row comparison against what is already stored, not the scan, and neither formulation avoids it. A model that re-offers every key on every tick is therefore bounded by its own size. Driving the refresh from what changed instead needs an arrival axis monotonic in commit order, which is `ingest_xid`, not `created_at`.
