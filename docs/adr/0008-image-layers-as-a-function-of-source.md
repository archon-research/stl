...
- **Base images are pinned by digest, package installs are not.** Every `FROM` names a tag and a digest together (VEC-783), and `scripts/deploy/check-base-image-version-consistency.sh` holds those tags to `.go-version`/`.python-version`/`.node-version` and to each other, so a rebuild resolves the same base until a pin changes in a diff. What a rebuild does not resolve the same way is the package layer: `apt-get upgrade -y` and unpinned `apk add` install whatever the index serves that day, so two builds of one commit still diverge over time (VEC-814). ADR-0006 §2 retains the original image as the guaranteed fallback for that reason.
