...
- Ingested by the **psm3-indexer** (`cmd/workers/psm3-indexer`, service `internal/services/psm3`): per-chain SQS block events → sweep every N blocks → append-only `psm3_reserves` rows (raw integers + `conversion_rate`, plus the ALM proxy's share legs: `alm_shares`, `total_shares`, `alm_asset_value`). USD math lives in the Python API.
