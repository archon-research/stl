...
- Ingested by the **psm3-indexer** (`cmd/workers/psm3-indexer`, service `internal/services/psm3`): per-chain SQS block events → sweep every N blocks → append-only `psm3_reserves` rows (raw integers + `conversion_rate`, plus `total_shares`; per-ALM stakes go to `psm3_alm_shares` (one row per tracked prime per sweep: `prime_id`, `alm_address`, `shares`, `asset_value`)). USD math lives in the Python API.
