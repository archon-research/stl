...
| `reference-core-indexer` | `reference-core-indexer` | 30m | Block Analitica's CORE model results (core.blockanalitica.com) → `reference_core_market_result` + `reference_core_vault_result`; reference data, the only writer of that history (VEC-828) |
...
## VectorReferenceCoreIndexerWritesZero

**Severity:** warning · **For:** 1h (over a 3h window)

**What it means.** Cycles are succeeding but `reference_core_market_result`
received no rows for three hours. `reference-core-indexer` is the only writer of
the CORE reference series, and the upstream dashboard
(`core.data.blockanalitica.com/core/overview/`) publishes one result per calendar
...
This alert also fires if `reference_core_sync_markets_written_total` stops
...
1. Confirm the worker is cycling rather than wedged:
   `kubectl -n vector logs deploy/reference-core-indexer --tail=100`. A healthy
...
2. Check the feed answers at all:
   `curl -s "$REFERENCE_CORE_URL/overview/" | jq '.data.markets | length, .data.vaults | length'`.
...
3. If the feed and the logs both look healthy, the counter is the fault: check
   ...
   `internal/services/reference_core_indexer/telemetry.go` still match this rule.

**Resolution.** A shape drift is fixed in the client
(`internal/adapters/outbound/blockanalitica/core_client.go`); its required-field
...
## VectorReferenceCoreIndexerStaleUpstream

**Severity:** warning · **For:** 2h (over a 3h window)

**Nothing is broken in STL.** Cycles succeed and rows land, but every one of
them carries an old `model_date`: Block Analitica's CORE model has stopped
...
`reference_core_sync_stale_cycles_total` once per such cycle.
...
other row was current. `reference_core_sync_stale_rows_total` counts those and
...
1. Read the latest date the feed reports:
   `curl -s "$REFERENCE_CORE_URL/overview/" | jq '.data.kpis.latest_run_date'`.
   If it is two or more days behind today (UTC), upstream is stuck and this is
   confirmed.
2. Cross-check with the stored rows:
   `SELECT max(model_date), max(synced_at) FROM reference_core_market_result;`
   `max(model_date)` should lag `max(synced_at)` by at most a day in health.
3. Look at `core.blockanalitica.com/system` (their own cron health page) for
   a failing liquidator or forecaster run.

**Resolution.** Flag it to Block Analitica; there is nothing to fix here. Do
...
## VectorReferenceCoreResultGrowthHigh
...
`reference_core_market_result` and `reference_core_vault_result` receive one row
...
`REFERENCE_CORE_SYNC_INTERVAL` first.

### First checks

```sql
SELECT count(*), min(synced_at), max(synced_at) FROM reference_core_market_result;
SELECT count(*), min(synced_at), max(synced_at) FROM reference_core_vault_result;
```
...
1. `SELECT create_hypertable('reference_core_market_result', 'synced_at', chunk_time_interval => INTERVAL '7 days', migrate_data => true);`
