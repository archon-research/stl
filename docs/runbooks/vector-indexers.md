...
The scan knobs are the Deployment's ConfigMap: `FINALITY_DEPTH`, `INITIAL_WINDOW`,
`MIN_WINDOW`, `MAX_WINDOW`, `POSITION_BATCH` — optional and defaulted when unset,
except `FINALITY_DEPTH` off mainnet (below). Changing one is a config change and a
