...
if [ ! -f "$SCRIPT_DIR/.env" ]; then
  ...
  echo ""
  echo "⚠️  IMPORTANT: Edit .env and fill in your credentials before bootstrap:"
  echo "   Required:"
  ...
  echo "     - Tailscale: TAILSCALE_AUTH_KEY (for staging/prod bastion)"
  echo ""
else
...
echo ""
echo "⚠️  IMPORTANT: Before bootstrap, ensure .env has required credentials:"
echo "   - TIGERDATA_PROJECT_ID, TIGERDATA_ACCESS_KEY, TIGERDATA_SECRET_KEY"
echo "   - ALCHEMY_API_KEY"
echo "   - Optional: COINGECKO_API_KEY, ETHERSCAN_API_KEY, TAILSCALE_AUTH_KEY"
echo ""
echo "Deployment workflow:"
echo "  1. Edit .env with your API keys and credentials"
echo "  2. source .env  # Load credentials into environment"
echo "  3. make tf-bootstrap ENV=<env>  # Creates S3 backend + secrets"
