...
- Every migration must self-register: end it with `INSERT INTO migrations(filename) … ON CONFLICT DO NOTHING` or it re-runs on every invocation. Don't mask that with an idempotency guard.
