...
- **Never commit generated files or binaries.** Every tool's skill deploy target
  ...
  and the `Skillfile`/`Skillfile.lock`. One deliberate exception: the generated
  `images:` and `configMapGenerator:` blocks of `k8s/overlays/{staging,prod}/kustomization.yaml`,
  plus the generated `images:` blocks of their `bootstrap/` overlays, are committed
  by the deploy bot — never by you (see `k8s/AGENTS.md`, ORB-362).
