...
- **Never commit generated files or binaries.** Every tool's skill deploy target
  ...
  and the `Skillfile`/`Skillfile.lock`. One deliberate exception: the `images:`
  blocks of `k8s/overlays/{staging,prod}/kustomization.yaml` are generated *and*
  committed — written by the deploy bot from `k8s/image-roster.txt`, never by you
